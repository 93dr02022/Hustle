...
class ProjectController extends Controller
{
    ...
    public function create(ProjectRequest $request)
    {
        ...
        if ($request->ajax()) {
            ...
            try {
                ...
                if ($deduction > auth()->user()->balance_available) {
                    return response()->json([
                ...
                $project = Project::create([
                    ...
                    'is_featured' => $premium['is_featured'],
                    'is_urgent' => $premium['is_urgent'],
                    'is_highlighted' => $premium['is_highlighted'],
                    'is_alert' => $premium['is_alert'],
                    'budget_allocation' => $budgetAllocation,
                    'expiry_date_featured' => $this->featuredDate($premium['is_featured']),
                    'expiry_date_urgent' => $this->urgentDate($premium['is_urgent']),
                    'expiry_date_highlight' => $this->highlightDate($premium['is_highlighted']),
                    'status' => $status,
                ]);

                // here we will charge the employer the project amount
                User::where('id', auth()->id())
                    ->update(['balance_available' => DB::raw("balance_available - $budgetAllocation")]);
     ...
     */
    public function featuredDate($isFeatured)
    {
        if ($isFeatured) {
     ...
     * @return Carbon\Carbon | null
     */
    public function urgentDate($isUrgent)
    {
        if ($isUrgent) {
     ...
     * @return Carbon\Carbon | null
     */
    public function highlightDate($isHighlight)
    {
        if ($isHighlight) {
