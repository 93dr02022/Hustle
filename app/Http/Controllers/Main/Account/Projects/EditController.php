...
class EditController extends Controller
{
    ...
    public function form($id)
    {
        ...
        $skills = ProjectSkill::where('category_id', $project->category_id)->orderBy('name', 'desc')->get();
    ...
    public function update(ProjectRequest $request, $id)
    {
        ...
        if ($request->ajax()) {
            ...
            try {
                ...
                // here we will calculate previous amount charged + wallet balance
                // compared to the new amount the user wants to budget for project
                $tempUserWallet = $project->budget_allocation + auth()->user()->balance_available;

                // budget allocation
                if ($request->salary_type == 'fixed') {
                    $budgetAllocation = $request->input('price_min');
                    $deduction = $premium['total'] + $request->input('price_min');
                } else {
                    $budgetAllocation = $request->input('price_min') * 4;
                    $deduction = $premium['total'] + $request->input('price_min') * 4;
                }

                if ($deduction > $tempUserWallet) {
                    return response()->json([
                        'message' => 'Your wallet balance is insufficient to create this project.',
                    ], 409);
                }
                ...
                Project::where("id", $project->id)
                    ->update([
                        ...
                        'budget_min' => $request->get('price_min'),
                        'budget_max' => $request->get('price_max'),
                        'budget_type' => $request->get('salary_type'),
                        'is_featured' => $premium['is_featured'] ?? $project->is_featured,
                        'is_urgent' => $premium['is_urgent'] ?? $project->is_urgent,
                        'is_highlighted' => $premium['is_highlighted'] ?? $project->is_highlighted,
                        'is_alert' => $premium['is_alert'] ?? $project->is_alert,
                        'budget_allocation' => $budgetAllocation,
                        'expiry_date_featured' => $this->featuredDate($premium['is_featured'], $project),
                        'expiry_date_urgent' => $this->urgentDate($premium['is_urgent'], $project),
                        'expiry_date_highlight' => $this->highlightDate($premium['is_highlighted'], $project),
                        'status' => $status
                    ]);

                // here will update the employer account balance to the new one 
                // due to the fact that we are doing an update here not first time charge.
                User::where('id', auth()->id())
                    ->update(['balance_available' => $tempUserWallet - $deduction]);
                ...
                if ($settings->is_premium_posting && is_array($request->get('plans', [])) && count($request->get('plans', []))) {
                    ...
                    $subscription->payment_id = mt_rand(1111111111, 9999999999);
                ...
                // Flash a message
                session()->flash('success', __('messages.t_ur_project_created_success'));
    ...
    /**
     * Calculate project promotion expiration date
     * 
     * @return Carbon\Carbon | null
     */
    public function featuredDate($isFeatured, $project)
    {
        if ($isFeatured) {
        ...
        if (!$featured_plan) {
            return $project->expiry_date_featured;
        }

        if ($project->expiry_date_featured) {
            return now()->parse($project->expiry_date_featured)->addDays($featured_plan->days);
        } else {
            return now()->addDays($featured_plan->days);
        }
    }
     ...
     * @return Carbon\Carbon | null
     */
    public function urgentDate($isUrgent, $project)
    {
        if ($isUrgent) {
        ...
        if (!$urgent_plan) {
            return $project->expiry_date_urgent;
        }

        if ($project->expiry_date_urgent) {
            return now()->parse($project->expiry_date_urgent)->addDays($urgent_plan->days);
        } else {
            return now()->addDays($urgent_plan->days);
        }
    }
     ...
     * @return Carbon\Carbon | null
     */
    public function highlightDate($isHighlight, $project)
    {
        if ($isHighlight) {
        ...
        if (!$highlighted_plan) {
            return $project->expiry_date_highlight;
        }

        if ($project->expiry_date_highlight) {
            return now()->parse($project->expiry_date_highlight)->addDays($highlighted_plan->days);
        } else {
            return now()->addDays($highlighted_plan->days);
        }
    }
