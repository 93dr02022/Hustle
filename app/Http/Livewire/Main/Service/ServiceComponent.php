...
class ServiceComponent extends Component
{
    ...
    public function mount($slug)
    {
        ...
        } elseif ($gig->status == 'pending' && auth()->check() && auth()->id() == $gig->user_id) {

            $this->setGig();

        } elseif (in_array($gig->status, ['active', 'featured', 'boosted', 'trending'])) {
    ...
    protected function setGig()
    {
        ...
        Track::dispatch([
            ...
            'utm_medium' => request()->get('utm_medium') ? str_replace([':', '\\', '/', '*', ',', '"', "'", ';'], ' ', mb_convert_encoding(request()->get('utm_medium'), 'UTF-8', 'UTF-8')) : null,
            'utm_source' => request()->get('utm_source') ? str_replace([':', '\\', '/', '*', ',', '"', "'", ';'], ' ', mb_convert_encoding(request()->get('utm_source'), 'UTF-8', 'UTF-8')) : null,
            'utm_campaign' => request()->get('utm_campaign') ? str_replace([':', '\\', '/', '*', ',', '"', "'", ';'], ' ', mb_convert_encoding(request()->get('utm_campaign'), 'UTF-8', 'UTF-8')) : null,
