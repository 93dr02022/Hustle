...
class OrderItemRefunded extends Notification implements ShouldQueue
{
    ...
    {
        // Set subject
        $subject = "[" . config('app.name') . "] " . __('messages.t_notification_buyer_refund_accepted');

        return (new MailMessage)
            ->subject($subject)
            ->greeting(__('messages.t_hello_username', ['username' => $notifiable->username]))
            ->line(__('messages.t_refunded'))
            ->action(__('messages.t_my_orders'), url('account/orders'));
    }
    public function toFirebase($notifiable)
    {
        // Set subject
        $subject = "[" . config('app.name') . "] " . __('messages.t_refunded');
        $url =  url('account/orders');
        return  Larafirebase::withTitle($subject)
            ->withBody(__('messages.t_refunded'))
            ->withClickAction($url)
