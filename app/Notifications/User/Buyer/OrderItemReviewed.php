...
class OrderItemReviewed extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    {
        // Set subject
        $subject = "[" . config('app.name') . "] " . __('messages.t_request_review');

        return (new MailMessage)
            ->subject($subject)
            ->greeting(__('messages.t_hello_username', ['username' => $notifiable->username]))
            ->line(__('messages.t_order_reviewed'))
    ...
    public function toFirebase($notifiable)
    {
        ...
        return  Larafirebase::withTitle($subject)
            ->withBody(__('messages.t_order_reviewed'))
            ->withClickAction($url)
