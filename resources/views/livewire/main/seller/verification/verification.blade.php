            ...
            <div class="py-6 px-4 sm:p-6 lg:pb-8 h-[calc(100%-80px)]">
                ...
                <div class="mb-9">
                    <h2 class="text-base leading-6 font-bold text-gray-900 dark:text-gray-100">
                        {{ __('messages.t_verification_center') }}</h2>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">
                        {{ __('messages.t_verification_center_subtitle') }}</p>
                </div>
                ...
                <div @if (!$verification->has_personal) @click="startVerify('personal')" @endif
                    ...
                    <div class="flex flex-col grow">
                        ...
                        <small class="text-gray-400 text-[12px] xs:text-sm">This is required in other to get store
                    ...
                    <svg width="12" height="17" class="flex-shrink-0 hidden xs:block" viewBox="0 0 9 14"
                        fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0 12.5825L1.51406 14L9 7V7V7L1.51406 0L0 1.4175L5.96719 7L0 12.5825Z" fill="#CACACA">
                        </path>
                    </svg>
                </div>
                ...
                <div @if ($verification->photo_status == 'pending' || $verification->photo_status == 'declined') @click="startVerify('photo')" @endif
                    class="py-4 lg:py-5 rounded cursor-pointer border card-shadow px-3 sm:px-4 flex items-start sm:items-center gap-x-3 mb-5">
                    <div class="bg-white grid place-items-center flex-shrink-0 shadow rounded p-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="#ffb14a"
                            class="text-[#ffb14a]" viewBox="0 0 16 16">
                            <path
                    ...
                    <svg width="12" height="17" class="flex-shrink-0 hidden xs:block" viewBox="0 0 9 14"
                        fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0 12.5825L1.51406 14L9 7V7V7L1.51406 0L0 1.4175L5.96719 7L0 12.5825Z"
                            fill="#CACACA"></path>
                    </svg>
                </div>
                ...
                <div @if ($verification->business_verify_status == 'pending' || $verification->business_verify_status == 'declined') @click="startVerify('business')" @endif
                    ...
                    <div class="flex flex-col grow">
                        ...
                        <small class="text-gray-400 text-[12px] xs:text-sm">business verification makes you eligible for
                            other store benefits.</small>
                        <div class="">
                ...
                <div class="">
                    ...
                    <div x-show="selectedVerification == 'personal'">
                        <form wire:submit.prevent>
                            <div class="grid md:grid-cols-2 gap-4 mb-6">
                                <x-forms.text-input label="{{ __('BVN Number') }}"
                                    placeholder="{{ __('12345678910') }}" model="bvn" type="tel"
                                    icon="at" required />

                                <x-forms.text-input label="{{ __('Account Number') }}"
                                    placeholder="{{ __('1234567890') }}" model="accountNumber" type="tel"
                                    icon="at" required />

                                <div class="w-full" wire:ignore>
                                    <x-forms.select2 :label="__('Select Your bank')" :placeholder="__('Select Bank')" model="bank"
                                        :options="$banks" :isDefer="true" :isAssociative="false" :componentId="$this->id"
                                        value="code" text="name" required />
                                </div>

                                <x-forms.text-input label="{{ __('First name') }}"
                                    placeholder="{{ __('first name') }}" model="first_name" type="tel"
                                    icon="at" readonly required />

                                <x-forms.text-input label="{{ __('Last name') }}"
                                    placeholder="{{ __('Last name') }}" model="last_name" type="tel"
                                    icon="at" class="disabled:bg-slate-300" readonly required />
                            </div>
                            <div class="border-t my-3 pt-3 flex justify-end">
                                <button type="submit" wire:click="personalVerify" wire:loading.attr="disabled"
                                    wire:target="personalVerify" class="btn-purple">Submit</button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div x-show="selectedVerification == 'business'">
                        <form @submit.prevent="submitBusiness">
                            <div class="grid md:grid-cols-2 gap-4 mb-6">
                                <div class="">
                                    <x-forms.text-input label="{{ __('Business Name') }}"
                                        placeholder="{{ __('shola store') }}" x-model="bizForm.business_name"
                                        type="text" icon="at" required />
                                    <p x-show="bizFormErrors?.business_name"
                                        class="mt-1 text-xs text-red-600 dark:text-red-500"
                                        x-text="bizFormErrors?.business_name?.[0]"></p>
                                </div>

                                <div class="">
                                    <x-forms.text-input label="{{ __('Business email') }}"
                                        placeholder="{{ __('sholastore@gmail.com') }}"
                                        x-model="bizForm.business_email" type="email" icon="at" required />
                                    <p x-show="bizFormErrors?.business_email"
                                        class="mt-1 text-xs text-red-600 dark:text-red-500"
                                        x-text="bizFormErrors?.business_email?.[0]"></p>
                                </div>

                                <div class="">
                                    <x-forms.text-input label="{{ __('Business phone') }}"
                                        placeholder="{{ __('0814525555') }}" maxlength="11" minlength="11"
                                        x-model="bizForm.business_phone" type="tel" icon="at" required />
                                    <p x-show="bizFormErrors?.business_phone"
                                        class="mt-1 text-xs text-red-600 dark:text-red-500"
                                        x-text="bizFormErrors?.business_phone?.[0]"></p>
                                </div>

                                <div class="">
                                    <x-forms.text-input label="{{ __('RC Number') }}"
                                        placeholder="{{ __('5252584425') }}" x-model="bizForm.registration_number"
                                        type="text" icon="at" required />
                                    <p x-show="bizFormErrors?.registration_number"
                                        class="mt-1 text-xs text-red-600 dark:text-red-500"
                                        x-text="bizFormErrors?.registration_number?.[0]"></p>
                                </div>

                                <div class="col-span-2">
                                    <div class="col-span-2 border-2 border-dashed rounded-md px-3 py-4 mb-4 mt-5">
                                        <label for="bizfile"
                                            class="block text-[0.8125rem] mb-2 font-medium tracking-wide text-gray-700 dark:text-white">CAC/BN
                                            Certificate</label>
                                        <input type="file" id="bizfile"
                                            ...
                                            accept="image/jpg,image/jpeg,image/png" @change="handleRegFile($event)"
                                            required>
                                        <p x-show="bizFormErrors?.registration_file"
                                            class="mt-1 text-xs text-red-600 dark:text-red-500"
                                            x-text="bizFormErrors?.registration_file?.[0]"></p>
                                    </div>
                                </div>
                            </div>
                            <div class="border-t my-3 pt-3 flex justify-end">
                                <button type="submit" wire:loading.attr="disabled" wire:target="businessVerify"
                                    :disabled="bizForm.registration_file == null" class="btn-purple">Submit</button>
                            </div>
                        </form>
                    </div>
                    ...
                    @if ($verification->photo_status == 'pending' || $verification->photo_status == 'declined')
                        <div class="" x-show="selectedVerification == 'photo'">
                                ...
                                <div class="flex">
                                    ...
                                    <div class="ltr:ml-3 rtl:mr-3">
                                        <p class="text-sm text-yellow-700">
                                            {{ __('messages.t_verification_selfie_page_msg') }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4" wire:ignore>
                                ...
                                <div class="">
                                    <div
                                        class="flex justify-center border-dashed border-2 border-gray-200 dark:border-zinc-700 mt-8 mb-4">
                                ...
                                {{-- Image taken --}}
                                <div
                                    class="flex items-center justify-center border-dashed border-2 border-gray-200 dark:border-zinc-700 mt-8 mb-4">
                            ...
                            <div>
                                <button type="button"
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg"
                                        class="w-4 h-4 ltr:mr-2 rtl:ml-2 ltr:-ml-1 rtl:-mr-1" fill="none"
                            ...
                            {{-- upload photo --}}
                            <div class="border-t my-3 pt-3 flex justify-end">
                                <button @click="upload()" :disabled="selfie.length <= 0" wire:loading.attr="disabled"
                                    wire:target="photoVerify" class="btn-purple">Submit</button>
                            </div>
                        </div>
                    @endif
                </div>
