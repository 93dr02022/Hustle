<div class="w-full">
    ...
    <div class="px-4 mx-auto space-y-2 max-w-7xl sm:px-6 md:px-12">
        <div class="max-w-3xl mx-auto" x-data="window.SellerDashboardCreateQuote">
            {{-- notice message --}}
            <div class="bg-yellow-50 ltr:border-l-4 rtl:border-r-4 border-yellow-400 py-7 px-4 mb-4">
                <div class="flex">
                    ...
                    <div class="ltr:ml-3 rtl:mr-3">
                        <p class="text-sm text-yellow-700">
                            The allowed minimum quotation items is NGN 10,000, only quotation above this price can be submitted.
                        </p>
                    </div>
                </div>
            </div>

            <div x-show="!successful" class="pt-5 bg-white rounded shadow-md">
                <form x-on:submit.prevent="createQuotation">
                                ...
                                <div class="grid grid-cols-2 gap-y-2 gap-x-3 sm:gap-x-5 md:grid-cols-3">
                                    <div class="col-span-2 md:col-span-4">
                                        <div class="flex justify-between mb-1">
                                            <label class="mb-0 label-text"><span x-text="`${index + 1}.`"></span> Item
                                                Description</label>
                                    ...
                                    <div class="">
                                        ...
                                        <input type="number" placeholder="Price" x-model="form.items[index].price" @input="sumTotal(index)" class="form-ctr" min="100" max="1000000"
                                            required>
                                    </div>
                    ...
                    <div class="divide-y dark:divide-gray-700 border-y dark:border-gray-700">
                        <div class="grid grid-cols-2 gap-3 px-3 py-6 sm:gap-5 sm:px-6">
                            <div class="">
                                ...
                                <input type="date" x-model="form.expires_at" class="form-ctr" min="{{ $expiration }}" required>
                            </div>
                            ...
                            <div x-show="form.payment_method == 'cash'" class="col-span-2 bg-gray-100 border flex items-center gap-x-3  rounded py-5 px-3" x-cloak>
                                ...
                                <label for="" class="text-sm text-gray-900">
                                    This is to affirm you have received cash payment from the quote recipient. Note this
                                    is a one time action and store commission will be deducted from your wallet.
                                </label>
                            </div>
                    ...
                    <div class="flex items-center justify-end px-4 py-3 text-right shadow bg-gray-50 dark:bg-black/50 sm:px-5 sm:rounded-bl-md sm:rounded-br-md">
                        <button type="submit" wire:loading.attr="disabled" wire:target="create" :disabled="total < 10000" class="btn-purple">Proceed</button>
                    </div>
                </form>
            </div>
            ...
            <div x-show="successful" class="max-w-md py-5 mx-auto bg-white rounded shadow-md" x-cloak>
                ...
                <div x-show="form.payment_method == 'paystack'" class="relative mt-7 mx-5">
                    ...
                    <div class="flex absolute inset-y-0 right-0 items-center pl-4 pr-2 py-3 pointer-events-auto">
                        <button @click="copy(`https://correcthustle.com/quotations/${quote.reference}/payment`)" :class="{ '!bg-green-200': copying }"
                            class="text-blue-500 rounded flex border-none py-2 px-3">Copy</button>
                    </div>
                </div>
            </div>
        </div>
