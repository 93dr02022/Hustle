    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-12">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3 lg:gap-6">
            ...
            @foreach ($projects as $project)
                <div class="relative flex flex-col break-words rounded-xl shadow-sm"
                    wire:key="freelancer-portfolio-{{ $project->uid }}">

                    {{-- Thumbnail --}}
                    <img class="h-72 w-full rounded-lg object-cover object-center"
                        src="{{ src($project?->thumbnail_id) }}" alt="{{ $project->title }}">

                    {{-- Card content --}}
                    <div class="absolute inset-0 flex h-full w-full flex-col justify-end">
                        <div
                            class="space-y-1.5 rounded-lg bg-gradient-to-t from-[#00000091] via-[#000000cc] to-transparent px-4 pb-3 pt-12">

                            {{-- Project title --}}
                            <div class="line-clamp-2">
                                <a href="{{ url('profile/' . $project->user->username . '/portfolio/' . $project->slug) }}"
                                    target="_blank" class="text-base font-medium text-white">
                                    {{ $project->title }}
                                </a>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center text-xs text-slate-200">
                                    ...
                                    @if ($project->status === 'pending')
                                        <span
                                            class="inline-flex items-center rounded-full text-xs font-medium text-amber-300">
                                            {{ __('messages.t_pending') }}
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center rounded-full text-xs font-medium text-green-400">
                                ...
                                <div class="ltr:-mr-1.5 rtl:-ml-1.5 flex space-x-2 rtl:space-x-reverse">
                                    ...
                                    <a href="{{ url('seller/portfolio/edit', $project->uid) }}"
                                        class="h-7 w-7 rounded-full p-0 hover:bg-slate-300/20 flex items-center justify-center">
                                        <svg class="w-4 h-4 text-slate-100" stroke="currentColor" fill="currentColor"
                                    ...
                                    <button
                                        ...
                                        wire:loading.attr="disabled" wire:target="delete('{{ $project->uid }}')"
                                        type="button"
                                        ...
                                        <div wire:loading wire:target="delete('{{ $project->uid }}')">
                                            <svg role="status" class="w-4 h-4 text-gray-500 animate-spin"
                                        ...
                                        <div wire:loading.remove wire:target="delete('{{ $project->uid }}')">
                                            <svg class="w-4 h-4 text-slate-100" stroke="currentColor"
            ...
            <a href="{{ url('seller/portfolio/create') }}"
                class="h-72 rounded-xl border-dashed border-2 border-slate-200 bg-opacity-20 bg-slate-100 hover:bg-opacity-100 dark:border-zinc-600 dark:bg-zinc-800 dark:hover:bg-zinc-700 flex items-center justify-center flex-col">
                <svg class="h-14 w-14 text-slate-300 dark:text-zinc-500" stroke="currentColor" fill="currentColor"
                    stroke-width="0" viewBox="0 0 24 24" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd"
