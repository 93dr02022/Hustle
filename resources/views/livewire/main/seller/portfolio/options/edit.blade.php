<div class="w-full">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 mb-10">
        <nav class="justify-between px-4 py-3 text-gray-700 border border-gray-100 rounded-lg shadow-sm sm:flex sm:px-5 bg-white dark:bg-zinc-800 dark:border-zinc-700 dark:shadow-none"
            ...
            <div>
                <a href="{{ url('seller/portfolio') }}"
                    class="flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 dark:bg-zinc-700 dark:shadow-none dark:border-zinc-700 dark:text-zinc-200 dark:hover:text-white dark:hover:bg-zinc-900 dark:hover:border-zinc-900">
                ...
                <div class="overflow-hidden rounded-lg bg-white dark:bg-zinc-800 shadow px-10 py-12">
                    <div class="grid col-span-12 md:gap-x-6 gap-y-8">
                        ...
                        <div class="col-span-12">
                            <x-forms.text-input :label="__('messages.t_project_title')" :placeholder="__('messages.t_enter_project_title')" model="title" maxlength="100"
                        ...
                        {{-- Project description --}}
                        <div class="col-span-12">
                            <x-forms.textarea :label="__('messages.t_project_description')" :placeholder="__('messages.t_enter_project_description')" model="description" :rows="10"
                                maxlength="5000"
                        ...
                        {{-- Project link --}}
                        <div class="col-span-12">
                            <x-forms.text-input :label="__('messages.t_project_link_optional')" :placeholder="__('messages.t_https_example_com')" model="link" maxlength="120"
                        ...
                        {{-- Project video --}}
                        <div class="col-span-12">
                            <x-forms.text-input :label="'Project video link (optional)'" :placeholder="__('messages.t_youtube_placeholder')" model="video" maxlength="120"
                        ...
                        <div class="col-span-12">
                            <x-forms.file-input :label="__('Project Video Upload')" model="videoFile" accept=".mp4, .mp3, .webm"
                                svg_icon='<svg class="w-5 h-5 text-gray-400 dark:text-gray-300" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M18 7c0-1.103-.897-2-2-2H4c-1.103 0-2 .897-2 2v10c0 1.103.897 2 2 2h12c1.103 0 2-.897 2-2v-3.333L22 17V7l-4 3.333V7zm-1.998 10H4V7h12l.001 4.999L16 12l.001.001.001 4.999z"></path></svg>' />
                        </div>

                        {{-- uploaded video --}}
                        @if($project->project_video_upload)
                            <video width="250" height="150" class="rounded-lg" controls>
                                <source src="{{ "https://hustlebucket.s3.amazonaws.com/{$project->project_video_upload}" }}" type="{{ $this->mime($project->project_video_upload) }}">
                            </video>
                        @endif
                    </div>
                </div>
                ...
                <div class="overflow-hidden rounded-lg bg-white dark:bg-zinc-800 shadow px-10 py-12 mb-6">
                    <div class="grid col-span-12 md:gap-x-6 gap-y-8">
                        ...
                        <div class="col-span-12">
                            <div wire:ignore>
                                <span
                                    class="mb-2 text-[0.8125rem] font-semibold tracking-wide flex items-center text-gray-700 dark:text-white">{{ __('messages.t_project_thumbnail') }}</span>
                                <x-forms.uploader model="thumbnail" id="uploader_thumbnail" :extensions="['jpg', 'jpeg', 'png']"
                                    accept="image/jpg, image/jpeg, image/png"
                                    size="{{ settings('media')->portfolio_max_size }}" max="1" />
                            </div>
                            @error('thumbnail')
                            ...
                            {{-- Preview image --}}
                            <div class="mt-3">
                                <img src="{{ src($project?->thumbnail_id) }}" alt="{{ $project->title }}"
                                    class="h-32 w-32 rounded-lg cursor-pointer object-cover">
                            </div>
                        ...
                        <div class="col-span-12">
                            <div wire:ignore>
                                <label
                                    class="mb-2 text-[0.8125rem] font-medium tracking-wide flex items-center {{ $errors->first('images') ? 'text-red-600' : 'text-gray-700 dark:text-white' }}">{{ __('messages.t_project_images') }}</label>
                            ...
                            <div class="grid grid-cols-12 gap-x-4 gap-y-6">
                                @foreach ($project->gallery as $img)
                                    <div class="col-span-6">
                                        <img src="{{ src($img->image_id) }}" alt="{{ $project->title }}"
                                            class="rounded-lg w-full h-32 object-cover intense cursor-pointer shadow-sm">
                                    </div>
                                @endforeach
                            </div>
                        </div>
