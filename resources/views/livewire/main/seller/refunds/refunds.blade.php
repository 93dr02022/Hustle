                ...
                <thead>
                    @forelse ($refunds as $refund)
                        <tr class="intro-x shadow-sm bg-white dark:bg-zinc-800 rounded-md h-16"
                            wire:key="freelancer-dashboard-refunds-{{ $refund->uid }}">
                                ...
                                <div class="flex items-center">
                                    <div class="h-10 w-10 flex-shrink-0">
                                        <img class="h-10 w-10 rounded-full lazy object-cover object-center"
                                            src="{{ placeholder_img() }}"
                                            data-src="{{ src($refund->buyer->avatar_id) }}"
                                            alt="{{ $refund->buyer->username }}">
                                    </div>
                                    <div class="ltr:ml-4 rtl:mr-4">
                                        <a href="{{ url('profile', $refund->buyer->username) }}" target="_blank"
                                            class="font-medium text-gray-900 dark:text-gray-200 pb-1 hover:text-primary-600 block text-sm">{{ $refund->buyer->username }}</a>
                            ...
                            <td
                                ...
                                @switch($refund->status)
                                    ...
                                    @case('pending')
                                        <span
                                            class="px-2.5 py-1 rounded-sm text-xs font-medium dark:bg-transparent text-amber-500 bg-amber-50">
                                    ...
                                    @case('closed')
                                        <span
                                            class="px-2.5 py-1 rounded-sm text-xs font-medium dark:bg-transparent text-gray-500 bg-gray-50">
                                    ...
                                    @case('rejected_by_seller')
                                        <span
                                            class="px-2.5 py-1 rounded-sm text-xs font-medium dark:bg-transparent text-red-500 bg-red-50">
                                    ...
                                    @case('accepted_by_seller')
                                        <span
                                            class="px-2.5 py-1 rounded-sm text-xs font-medium dark:bg-transparent text-green-500 bg-green-50">
                            ...
                            <td
                                ...
                                <div class="flex justify-center items-center space-x-2 rtl:space-x-reverse">
                                    ...
                                    {{-- Contact buyer --}}
                                    <a href="{{ url('messages/new', $refund->item->order->buyer->username) }}"
                                        target="_blank"
                                        data-tooltip-target="tooltip-actions-contact-buyer-{{ $refund->uid }}"
                                        ...
                                        wire:key="tooltip-actions-contact-buyer-{{ $refund->uid }}">
                                    ...
                                    <x-forms.tooltip id="tooltip-actions-contact-buyer-{{ $refund->uid }}"
                                        :text="__('messages.t_contact_buyer')" />

                                    {{-- Refund details --}}
                                    <a href="{{ url('seller/refunds/details', $refund->uid) }}"
                                        data-tooltip-target="tooltip-actions-refund-details-{{ $refund->uid }}"
                                        class="inline-flex justify-center items-center border font-semibold focus:outline-none w-8 h-8 leading-5 text-sm rounded border-gray-300 bg-white text-gray-800 shadow-sm hover:text-gray-800 hover:bg-gray-100 hover:border-gray-300 hover:shadow focus:ring focus:ring-gray-500 focus:ring-opacity-25 active:bg-white active:border-white active:shadow-none dark:bg-zinc-700 dark:border-zinc-700 dark:hover:bg-zinc-900 dark:hover:border-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100"
                                        wire:key="tooltip-actions-refund-details-{{ $refund->uid }}">
                                    ...
                                    <x-forms.tooltip id="tooltip-actions-refund-details-{{ $refund->uid }}"
