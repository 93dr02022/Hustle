...
<html lang="{{ app()->getLocale() }}" dir="{{ config()->get('direction') }}" @class(['dark' => current_theme() === 'dark', 'h-full'])>
	...
	<body class="h-full bg-slate-50 dark:bg-zinc-700 application scrollbar-thumb-gray-300 scrollbar-track-gray-100 dark:scrollbar-thumb-zinc-900 dark:scrollbar-track-zinc-600">
							...
							<div x-data="Components.menu({ open: false })" x-init="init()" @keydown.escape.stop="open = false; focusButton()" @click.away="onClickAway($event)" class="relative ltr:ml-3 rtl:mr-3">
								...
								<div 
									...
									<div class="py-1.5 px-3.5">
										...
										{{-- Referral --}}
										<a href="{{ url('account/referral') }}" class="group flex items-center py-1.5 group-hover:text-primary-600">
										 <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="currentColor" class="flex-none ltr:mr-3 rtl:ml-3 text-gray-400 group-hover:text-primary-600 h-4 w-4" viewBox="0 0 16 16">
											...
											<span class="font-semibold text-xs text-gray-700 dark:text-gray-100 group-hover:text-primary-600 dark:group-hover:text-primary-500">Referral</span>
										</a>
									</div>
