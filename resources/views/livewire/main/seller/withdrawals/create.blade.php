                ...
                <div class="mt-5 flex lg:mt-0 lg:ltr::ml-4 lg:rtl:mr-4">
                    ...
                    <span class="block">
                        <a href="{{ url('seller/withdrawals') }}"
                            ...
                            <svg class="h-5 w-5 text-gray-500 ltr:mr-2 rtl:ml-2 dark:text-zinc-200" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                stroke-width="2">
        ...
        <main class="rounded-lg bg-white dark:bg-zinc-800 shadow border border-gray-200 dark:border-zinc-800 py-12 px-[14px] sm:px-6 md:px-10">
            ...
            @if ($can_withdraw)
                <form wire:submit.prevent="withdraw(true)" class="grid grid-cols-2 md:gap-x-6 gap-y-6">
                    ...
                    <div class="">
                        @php
                            $userBalance = money(auth()->user()->balance_available, settings('currency')->code, true);
                        @endphp

                        <x-forms.text-input :label="__('messages.t_amount_to_withdraw')" placeholder="0.00" model="amount"
                            ...
                            :hint="__('messages.t_available_balance_amount', ['amount' => $userBalance])" required />
                    </div>

                    <div class="col-span-2">
                        <button type="submit" class="btn-purple w-full py-4">{{ __('messages.t_make_withdrawal') }}</button>
                    </div>
                </form>
            @else
                {{-- Error --}}
