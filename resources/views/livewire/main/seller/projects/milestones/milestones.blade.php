<div class="w-full" x-data="window.SellerMilestone">
                ...
                <div class="mt-5 flex lg:mt-0 lg:ltr::ml-4 lg:rtl:mr-4">
                    ...
                    @if (in_array($project->status, ['active', 'under_development']))
                        <span class="block sm:ltr:ml-3 sm:rtl:mr-3">
                            <button type="button" id="modal-request-milestone-button"
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-y-6 gap-x-5 h-full">
            ...
            <div>
                <div class="flex flex-col h-full justify-center text-center space-y-3 bg-white dark:bg-zinc-800 dark:border-transparent py-4 px-2 border rounded-md">
                    <svg class="mx-auto h-6 w-6 mb-1 text-slate-500 dark:text-zinc-200" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 20 20" aria-hidden="true"
            ...
            <table class="w-full text-left border-spacing-y-[10px] border-separate -mt-2">
                <thead class="">
                    <tr class="bg-slate-200 dark:bg-zinc-600">
                        ...
                        <th
                            class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md">
                            @lang('messages.t_paid_to_you')</th>

                        <th
                            class="font-bold tracking-wider text-gray-600 px-5 py-4.5 border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md">
                            @lang('Actions')</th>
                ...
                <thead>
                    @forelse ($payments as $p)
                        <tr class="shadow-sm bg-white dark:bg-zinc-800 rounded-md h-16" wire:key="freelancer-dashboard-project-milestones-{{ $p->uid }}">
                    ...
                    @if ($payments->count())
                        <tfoot>
                            <tr class="bg-slate-200 dark:bg-zinc-600 intro-x rounded-md h-16">
                                ...
                                <td class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md text-center"></td>
                            </tr>
                        </tfoot>
                    @endif
                </table>
                ...
                <x-slot name="content">
                    <div class="grid grid-cols-12 gap-y-6 gap-x-4 py-2">
                        ...
                        <div class="col-span-12">
                            <x-forms.text-input :label="__('messages.t_amount')" placeholder="0.00" model="amount"
                        ...
                        {{-- Description --}}
                        <div class="col-span-12">
                            <x-forms.textarea :label="__('messages.t_description')" :placeholder="__('messages.t_enter_milestone_payment_description')" model="description" :rows="6"
                ...
                <template x-if="milestone?.attachments?.length <= 0">
                    <div class="py-14 px-6 text-center text-sm sm:px-14">
                        ...
                        <p class="mt-2 text-gray-500 dark:text-zinc-400 max-w-md mx-auto">
                            click the button above to send files to employer</p>
                    </div>
                </template>
    ...
    @push('scripts')
        <script>
            ...
            window.SellerMilestone = SellerMilestone();

            window.addEventListener('close-right-modal', event => {
                window.SellerMilestone.hidden = false
            })
        </script>
