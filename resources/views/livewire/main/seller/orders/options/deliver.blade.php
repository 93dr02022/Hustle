<div class="w-full" x-data="window.TBhqVNUmCYEnOEj" x-on:livewire-upload-start="uploadStart()"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-12">
        <div class="grid grid-cols-12 md:gap-x-6 gap-y-6">
                                ...
                                <div class="min-w-0 flex-1 flex items-center space-x-3 rtl:space-x-reverse">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="text-sm font-medium text-gray-900 dark:text-gray-200 truncate pb-2">
                                            {{ json_decode($submited_work->review_attachment)->id }}.{{ json_decode($submited_work->review_attachment)->extension }}
                                        </p>
                                        <p class="text-xs font-[400] text-gray-400 truncate">
                                            {{ human_filesize(json_decode($submited_work->review_attachment)->size) }}
                                        </p>
                                    </div>
                                </div>
                                <div class="flex-shrink-0">
                                    <a href="{{ src(json_decode($submited_work->review_attachment)->path) }}" target="_blank"
                                        class="inline-flex items-center py-2 px-4 border border-transparent rounded-full bg-gray-100 hover:bg-gray-200 dark:bg-zinc-700 dark:hover:bg-zinc-600 focus:outline-none focus:ring-0"
                                        download>
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="ltr:mr-2 rtl:ml-2 h-5 w-5 text-gray-400" fill="none"
                    ...
                    @elseif (!$order->is_finished)
                        <div class="grid grid-cols-12 md:gap-x-6 gap-y-10">
                            ...
                            <div class="col-span-12" wire:ignore>
                                ...
                                {{-- Label --}}
                                <span
                                    class="mb-2 text-[0.8125rem] font-semibold tracking-wide flex items-center text-gray-700 dark:text-white">{{ __('messages.t_upload_work') }}</span>

                                {{-- Uploader --}}
                                <x-forms.uploader model="work" id="uploader_work" :extensions="['zip']"
                            ...
                            <div class="col-span-12">
                                <label for="deliver-work-quick-response"
                                    class="block text-[0.8125rem] font-semibold text-gray-700 dark:text-white">
                                    {{ __('messages.t_quick_response') }}
                                </label>
                                <div class="mt-2 relative">
                                    <textarea placeholder="{{ __('messages.t_describe_ur_delivery_in_detail') }}" wire:model.defer="quick_response"
                                        rows="8" id="deliver-work-quick-response"
                                    ...
                                    <div
                                        ...
                                        <svg class="text-gray-400 w-5 h-5" stroke="currentColor" fill="currentColor"
            ...
            <div class="col-span-12 lg:col-span-6">
                <div
                    class="bg-white dark:bg-zinc-900 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 mb-8">
                    ...
                    <div class="mb-6">
                        <div class="w-full">
                            <div class="mx-auto">
                                <div
                                    class="py-3 px-1 md:flex md:items-center md:justify-between lg:border-b lg:border-gray-200 lg:dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800 rounded-t-lg">
                                    <div class="flex-1 min-w-0 px-3">
                                        <div class="flex items-center">
                                            <img class="hidden h-10 w-10 rounded-full sm:block object-cover lazy"
                                                src="{{ placeholder_img() }}"
                                                data-src="{{ src($order->order->buyer->avatar_id) }}"
                                                alt="{{ $order->order->buyer->username }}">
                                            <div>
                                                <div class="flex items-center">
                                                    <h1
                                                        class="ltr:ml-3 rtl:mr-3 text-base font-bold leading-7 text-gray-900 dark:text-gray-200 sm:leading-9 sm:truncate">
                                    ...
                                    <div
                                        class="mt-6 flex space-x-3 rtl:space-x-reverse md:mt-0 md:ltr:mr-4 md:rtl:ml-4 ltr:ml-6 rtl:mr-6">
                                        <a href="{{ url('profile', $order->order->buyer->username) }}"
                                            target="_blank"
                                            class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-sm text-gray-700 bg-white hover:bg-gray-50 dark:bg-zinc-700 dark:hover:bg-zinc-600 dark:text-zinc-300 dark:border-zinc-700 focus:outline-none focus:ring-0">
