        ...
        <header class="border-b-2 border-gray-100 dark:border-zinc-700 py-8 rounded-t-md">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between">
                <div class="flex-1 min-w-0">
                    ...
                    {{-- Title --}}
                    <h1
                        class="mt-2 text-xl font-bold leading-7 text-gray-900 dark:text-gray-100 sm:text-2xl sm:truncate">
                        {{ __('messages.t_review_details') }}</h1>
                ...
                <div class="mt-5 flex xl:mt-0 xl:ltr:ml-4 xl:rtl:mr-4">
                    ...
                    <span class="block ltr:mr-3 rtl:ml-4">
                        <a href="{{ url('messages/new', $review->user->username) }}" target="_blank"
            ...
            <article class="md:gap-8 md:grid md:grid-cols-3 px-6 py-8">
                <div>
                    <div class="flex items-center mb-6 space-x-4 rtl:space-x-reverse">
                        <img class="w-10 h-10 rounded-full object-cover lazy" src="{{ placeholder_img() }}"
                            data-src="{{ src($review->user->avatar_id) }}" alt="{{ $review->user->username }}">
                        <div class="space-y-1 font-medium dark:text-white">
                            <p>{{ $review->user->username }}</p>
                            @if ($review->user->country)
                                <div class="flex items-center text-xs text-gray-500 dark:text-gray-400">
                                    <img src="{{ placeholder_img() }}"
                                        data-src="{{ url('img/flags/' . strtolower($review->user->country->code) . '.svg') }}"
                                        alt="{{ $review->user->country->name }}" class="lazy h-3.5 ltr:mr-2 rtl:ml-2">
                    ...
                    <ul class="space-y-4 text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <li class="flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 ltr:mr-1.5 rtl:ml-1.5"
                                viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd"
                                    d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z"
                                    clip-rule="evenodd" />
                        ...
                        <div class="pr-4">
                            <footer>
                                <p class="mb-2 text-sm text-gray-500 dark:text-gray-400">
                                    {{ format_date($review->created_at, 'ago') }}</p>
                            </footer>
                        </div>
                        <p
                            class="bg-blue-700 text-white text-sm font-semibold inline-flex items-center px-3 py-1.5 rounded">
                            {{ $review->rating }}</p>
                    </div>
