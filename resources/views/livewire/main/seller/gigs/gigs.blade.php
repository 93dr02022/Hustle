    ...
    <div wire:loading>
        <div class="fixed inset-0 flex items-end overflow-hidden justify-center sm:items-center z-50">
            <div class="w-full h-full flex items-center justify-center">
                <div
                    class="app-preloader fixed z-50 grid h-full w-full place-content-center inset-0 bg-secondary-400 bg-opacity-60 transform transition-opacity dialog-backdrop backdrop-blur-sm dark:bg-secondary-700 dark:bg-opacity-60">
            ...
            <div class="lg:flex lg:items-center lg:justify-between">
                ...
                <div class="mt-5 flex lg:mt-0 lg:ltr::ml-4 lg:rtl:mr-4 justify-between">
                    ...
                    <span class="sm:ltr:ml-3 sm:rtl:mr-3">
                        <a href="{{ url('create') }}"
                            class="inline-flex items-center rounded-sm border border-transparent bg-primary-600 px-4 py-2 text-[13px] font-semibold text-white shadow-sm hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-zinc-800 dark:focus:ring-zinc-800">
            ...
            <div class="rounded-md bg-green-100 dark:bg-zinc-700 p-4">
                <div class="flex">
                    <div class="flex-shrink-0">
                        <svg class="h-5 w-5 text-green-400 dark:text-zinc-500" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <div class="ltr:ml-3 rtl:mr-3">
                        <p class="text-sm font-medium text-green-800 dark:text-zinc-400">{{ session()->get('success') }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <table class="w-full text-left border-spacing-y-[10px] border-separate sm:mt-2">
                <thead class="">
                    <tr class="bg-slate-200 dark:bg-zinc-600">
                        ...
                        <th
                            class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md">
                            @lang('messages.t_sales')</th>

                        {{-- Clicks --}}
                        <th
                            class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md">
                            @lang('messages.t_clicks')</th>
                ...
                <thead>
                    @forelse ($gigs as $gig)
                        <tr class="intro-x shadow-sm bg-white dark:bg-zinc-800 rounded-md h-16"
                            wire:key="freelancer-dashboard-gigs-{{ $gig->uid }}">
                                ...
                                <div class="flex items-center space-x-3 rtl:space-x-reverse">
                                    ...
                                    <div class="h-10 w-10">
                                        <img class="w-full h-full rounded-md object-cover lazy"
                                            src="{{ placeholder_img() }}" data-src="{{ src($gig->image_thumb_id) }}"
                                            alt="{{ $gig->title }}">
                                    </div>
                                        ...
                                        <div class="text-slate-500 text-xs whitespace-nowrap mt-2">
                                            <nav class="flex" aria-label="Breadcrumb">
                                                <ol role="list"
                                                    ...
                                                    <li>
                                                        <div class="flex items-center">
                                                            <a href="{{ url('categories', $gig->category->slug) }}"
                                                                target="_blank"
                                                                class="text-xs font-normal text-gray-500 hover:text-gray-700 dark:text-gray-300 dark:hover:text-gray-100">{{ $gig->category->name }}</a>
                                                        </div>
                                                    </li>
                                                    ...
                                                    <li>
                                                        <div class="flex items-center">
                                                            ...
                                                            <a href="{{ url('categories/' . $gig->category->slug . '/' . $gig->subcategory->slug) }}"
                                                                target="_blank"
                            ...
                            <td
                                ...
                                <div class="text-gray-900 dark:text-gray-100 text-sm font-black">@money($gig->price, settings('currency')->code, true)
                                </div>
                                <div class="text-xs text-gray-400 tracking-wide font-normal mt-1 whitespace-nowrap">
                                    {{ __('messages.t_number_days_for_delivery', ['number' => $gig->delivery_time]) }}
                                </div>
                            </td>
                            ...
                            <td
                                ...
                                <div class="flex items-center justify-center mb-2 z-0"
                                    data-rating-value="{{ $gig->rating }}">
                                    {!! render_star_rating($gig->rating, '0.85rem', '0.85rem', '#d0d0d0') !!}
                                </div>
                                <div class="text-xs text-gray-400 tracking-wide font-normal mt-2.5 whitespace-nowrap">
                                    {{ __('messages.t_number_reviews', ['number' => number_format($gig->counter_reviews)]) }}
                                </div>
                            </td>
                            ...
                            <td
                                class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md text-center">
                                <span
                                    class="text-gray-900 dark:text-gray-100 text-sm font-black">{{ number_format($gig->counter_sales) }}</span>
                                @if ($gig->orders_in_queue == 1)
                                    <p class="text-xs text-gray-400 tracking-wide font-normal mt-1 whitespace-nowrap">
                            ...
                            <td
                                ...
                                <div class="text-xs text-gray-400 tracking-wide font-normal mt-1 whitespace-nowrap">
                                    ...
                                    {{ strtolower(__('messages.t_impressions')) }}
                                </div>
                            </td>
                            ...
                            <td
                                ...
                                @switch($gig->status)
                                    ...
                                    @case('pending')
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-sm text-xs font-medium bg-yellow-50 text-yellow-800 dark:bg-transparent dark:text-amber-400">
                                    ...
                                    @case('active')
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-sm text-xs font-medium bg-green-50 text-green-800 dark:bg-transparent dark:text-green-400">
                                    ...
                                    @case('deleted')
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-sm text-xs font-medium bg-red-50 text-red-800 dark:bg-transparent dark:text-red-400">
                                    ...
                                    @case('featured')
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-sm text-xs font-medium bg-purple-50 text-purple-800 dark:bg-transparent dark:text-purple-400">
                                    ...
                                    @case('trending')
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-sm text-xs font-medium bg-blue-50 text-blue-800 dark:bg-transparent dark:text-blue-400">
                                    ...
                                    @case('boosted')
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-sm text-xs font-medium bg-gray-50 text-gray-800 dark:bg-transparent dark:text-gray-300">
                            ...
                            <td
                                ...
                                <div class="flex justify-center items-center space-x-2 rtl:space-x-reverse">
                                    ...
                                    {{-- Edit --}}
                                    <a href="{{ url('seller/gigs/edit', $gig->uid) }}"
                                        data-tooltip-target="tooltip-actions-edit-{{ $gig->uid }}"
                                    ...
                                    <x-forms.tooltip id="tooltip-actions-edit-{{ $gig->uid }}"
                                        :text="__('messages.t_edit')" />

                                    {{-- Analytics --}}
                                    <a href="{{ url('seller/gigs/analytics', $gig->uid) }}"
                                        data-tooltip-target="tooltip-actions-analytics-{{ $gig->uid }}"
                                        class="inline-flex justify-center items-center border font-semibold focus:outline-none w-8 h-8 leading-5 text-sm rounded border-gray-300 bg-white text-gray-800 shadow-sm hover:text-gray-800 hover:bg-gray-100 hover:border-gray-300 hover:shadow focus:ring focus:ring-gray-500 focus:ring-opacity-25 active:bg-white active:border-white active:shadow-none dark:bg-zinc-700 dark:border-zinc-700 dark:hover:bg-zinc-900 dark:hover:border-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100">
                                        <svg class="w-4 h-4" stroke="currentColor" fill="currentColor"
                                            stroke-width="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <x-forms.tooltip id="tooltip-actions-analytics-{{ $gig->uid }}"
                                        :text="__('messages.t_analytics')" />

                                    {{-- Delete --}}
                                    <button wire:click="confirmDelete('{{ $gig->uid }}')" type="button"
                                        data-tooltip-target="tooltip-actions-delete-{{ $gig->uid }}"
                                        class="inline-flex justify-center items-center border font-semibold focus:outline-none w-8 h-8 leading-5 text-sm rounded border-gray-300 bg-white text-gray-800 shadow-sm hover:text-gray-800 hover:bg-gray-100 hover:border-gray-300 hover:shadow focus:ring focus:ring-gray-500 focus:ring-opacity-25 active:bg-white active:border-white active:shadow-none dark:bg-zinc-700 dark:border-zinc-700 dark:hover:bg-zinc-900 dark:hover:border-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100">
                                        <svg class="w-4 h-4" stroke="currentColor" fill="currentColor"
                                            stroke-width="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path
                                    ...
                                    <x-forms.tooltip id="tooltip-actions-delete-{{ $gig->uid }}"
                                        :text="__('messages.t_delete')" />
                        ...
                        {{-- Delete confirmation --}}
                        <x-forms.modal id="modal-delete-gig-container-{{ $gig->uid }}"
                            target="modal-delete-gig-button-{{ $gig->uid }}" uid="modal_{{ uid() }}"
                            ...
                            <x-slot name="content">
                                ...
                                {{-- Delete --}}
                                <div>
                                    <button wire:click="delete({{ $gig->id }})"
