<div class="w-full">
    ...
    {{-- Hero section --}}
    <div class="rounded-xl px-7 pt-2.5 lg:min-h-[530px] md:bg-[120%] xl:bg-[100%] bg-white shadow-sm border border-gray-100 bg-contain bg-no-repeat  relative mx-auto dark:bg-zinc-800 dark:border-transparent lg:bg-[image:var(--projects-image-url)]"
        style="--projects-image-url: url({{ url('img/explore/projects/1.jpg') }});">
        <div class="lg:max-w-xl w-full py-12">

            {{-- Headline --}}
            <h2
                class="mb-5 lg:leading-[62px] text-2xl leading-[32px] sm:text-3xl md:text-4xl lg:text-6xl font-bold text-zinc-900 dark:text-white">
            ...
            <div class="mt-5">
                @php
                    $popular_categories = App\Models\ProjectCategory::whereHas('projects')
                        ->withCount('projects')
                        ->take(5)
                ...
                @foreach ($categories as $category)
                    <a href="{{ url('explore/projects', $category->slug) }}"
                        ...
                        <span aria-hidden="true" class="absolute inset-0">
                            <img src="{{ src($category->thumbnail_id) }}" alt="{{ $category->name }}"
