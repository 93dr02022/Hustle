<div class="w-full">
    <div class="grid grid-cols-12 gap-6">
        ...
        {{-- Fatured categories --}}
        @if ($categories->count())
            <div class="bg-[#F1F4FF] col-span-12 mt-6 xl:mt-6 mb-16">
                <span
                    class="font-semibold text-gray-400 dark:text-gray-200 uppercase tracking-wider text-center block">{{ __('messages.t_featured_categories') }}</span>
                <div class="flex-wrap justify-center items-center mt-8 -mx-4 hidden" id="featured-categories-slick"
                    wire:ignore>
                    ...
                    @foreach ($categories as $category)
                        <a href="{{ url('categories', $category->slug) }}"
                            ...
                            <span aria-hidden="true"
                                class="absolute inset-x-0 bottom-0 h-2/3 bg-gradient-to-t from-[#F59E0B] opacity-90"></span>
                            <span
                                class="relative mt-auto text-center text-base md:text-lg font-semibold text-slate-100">{{ $category->name }}</span>
                        </a>
                    @endforeach
...
@push('styles')
    ...
    @if (settings('appearance')->is_featured_categories)
        <link rel="preload" href="//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.css" type="text/css"
