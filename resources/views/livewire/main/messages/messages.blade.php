<div class="container" x-data="window.mTUkVVaUJscRImp" x-init="init()">
            ...
            <div class="wrapper">
                ...
                <div class="conversation-area rtl:!border-r-0 rtl:border-l border-gray-200 dark:border-zinc-700">
                    ...
                    @foreach ($conversations as $c)
                        <a href="{{ url('messages', $c->uid) }}"
                            class="msg {{ $c->sender->isOnline() ? 'online' : 'offline' }}"
                            wire:key="conversation-id-{{ $c->uid }}">
                            <img class="msg-profile rtl:!mr-0 rtl:!ml-4 lazy" src="{{ placeholder_img() }}"
                                data-src="{{ src($c->sender->avatar) }}" alt="{{ $c->sender->username }}" />
                ...
                <div class="chat-area items-center justify-start pt-12" id="chat-area">
                    <div class="py-14 px-6 text-center text-sm sm:px-14">
                        ...
                        <p class="mt-4 font-semibold text-gray-900 dark:text-gray-200">
                            {{ __('messages.t_no_conversation_selected') }}</p>
                        <p class="mt-2 text-gray-500 dark:text-gray-400">
                            {{ __('messages.t_no_conversation_selected_subtitle') }}</p>
                    </div>
                </div>
            </div>
        </div>
    @else
        <div
            class="py-14 px-6 text-center text-sm sm:px-14 max-w-xl border-2 bg-slate-50 dark:bg-zinc-800 border-gray-200 dark:border-zinc-700 m-auto border-dashed">
            <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto h-6 w-6 text-gray-400 dark:text-gray-200"
            ...
            <p class="mt-2 text-gray-500 dark:text-gray-300">
                {{ __('messages.t_u_dont_have_any_active_conversations') }}</p>
        </div>
