        ...
        <div class="col-span-12 lg:col-span-4">
            <div class="border border-gray-100 rounded-lg bg-white dark:bg-zinc-800 dark:border-zinc-700">
                <div class="flex flex-col items-center py-10">
                    <img class="mb-3 w-24 h-24 rounded-full object-cover lazy" src="{{ placeholder_img() }}"
                        data-src="{{ src($user->avatar_id) }}" alt="{{ $user->username }}">
                    <h5
                        class="mb-1 text-base font-black text-gray-900 dark:text-gray-200 tracking-wide flex items-center">
                        {{ $user->username }}
                    ...
                    <div class="flex mt-4 space-x-3 rtl:space-x-reverse lg:mt-6">
                        ...
                        <a href="{{ url('messages/new', $user->username) }}"
                            class="inline-flex items-center py-3 px-4 text-xs font-medium text-center text-white bg-primary-600 rounded-sm hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300">{{ __('messages.t_contact_me') }}</a>

                        {{-- View profile --}}
                        <a href="{{ url('profile', $user->username) }}"
                            class="inline-flex items-center py-2 px-4 text-xs font-medium text-center text-gray-900 bg-white dark:bg-zinc-700 dark:text-zinc-400 dark:border-zinc-700 dark:hover:bg-zinc-600 rounded-sm border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200">{{ __('messages.t_view_profile') }}</a>
                            ...
                            <div class="justify-between sm:flex">
                                ...
                                <div class="flex-shrink-0 hidden ltr:ml-3 rtl:mr-3 sm:block">
                                    <img class="object-cover w-16 h-16 rounded-lg shadow-sm lazy"
                                        src="{{ placeholder_img() }}" data-src="{{ src($project?->thumbnail_id) }}"
                                        alt="{{ $project->title }}" />
                                </div>
                            </div>
                        </a>
                    ...
                    <div class="col-span-12">
                        <div class="border-dashed border-2 rounded-md mb-6">
                            <div class="py-14 px-6 text-center text-sm sm:px-14">
                                <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto h-6 w-6 text-gray-400"
                                ...
                                <p class="mt-2 text-gray-500 max-w-md mx-auto">
                                    {{ __('messages.t_this_user_has_no_projects_yet') }}</p>
                                <a href="{{ url('profile', $user->username) }}"
                                    class="mt-4 font-medium text-xs text-primary-600 hover:underline block">Back to
                                    profile</a>
                            </div>
                        </div>
                    </div>
