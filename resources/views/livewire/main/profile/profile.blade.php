                    ...
                    <div class="w-full p-2 py-10">
                        ...
                        <div class="flex justify-center">
                            <div class="relative">
                                ...
                                {{-- User avatar --}}
                                <img src="{{ placeholder_img() }}" data-src="{{ src($user->avatar_id) }}" alt="{{ $user->username }}" class="lazy rounded-full w-20 h-20 object-cover">
                        ...
                        <div class="flex flex-col text-center mt-3 mb-4">
                            <span class="text-md font-extrabold text-gray-800 dark:text-gray-100 flex items-center justify-center">
                                ...
                                @if ($user->status === 'verified')
                                    <img data-tooltip-target="tooltip-account-verified-{{ $user->id }}" class="ltr:ml-0.5 rtl:mr-0.5 h-4 w-4 -mt-0.5" src="{{ url('img/auth/verified-badge.svg') }}"
                        ...
                        @if ($user->country)
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 dark:text-gray-200 sm:mt-0 sm:col-span-2 flex items-center">
                                    <img src="{{ placeholder_img() }}" data-src="{{ url('img/flags/rounded/' . strtolower($user->country?->code) . '.svg') }}" alt="{{ $user->country?->name }}"
                                    ...
                                    @if ($project->status === 'active')
                                        <figure>
                                            <a href="{{ url('profile/' . $project->user->username . '/portfolio/' . $project->slug) }}" target="_blank">
                                                <img src="{{ placeholder_img() }}" data-src="{{ src($project?->thumbnail_id) }}" alt="{{ $project->title }}"
                    ...
                    <div class="border-t-2 border-gray-100 dark:border-zinc-700 px-4 py-5 sm:p-0">
                        <div class="px-6 py-5 space-x-2 space-y-4 text-center rtl:space-x-reverse">
                            ...
                            @if ($user->accounts->facebook_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->facebook_profile)) }}" target="_blank"
                            ...
                            @if ($user->accounts->twitter_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->twitter_profile)) }}" target="_blank"
                            ...
                            @if ($user->accounts->dribbble_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->dribbble_profile)) }}" target="_blank"
                            ...
                            @if ($user->accounts->stackoverflow_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->stackoverflow_profile)) }}" target="_blank"
                            ...
                            @if ($user->accounts->github_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->github_profile)) }}" target="_blank"
                            ...
                            @if ($user->accounts->youtube_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->youtube_profile)) }}" target="_blank"
                            ...
                            @if ($user->accounts->vimeo_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->vimeo_profile)) }}" target="_blank"
        ...
        <x-forms.modal id="modal-report-container" target="modal-report-button" uid="modal_{{ uid() }}" placement="center-center" size="max-w-md">
            ...
            <x-slot name="content">
                <div class="grid grid-cols-12 md:gap-x-6 gap-y-6">
                    ...
                    {{-- Message --}}
                    <div class="col-span-12">
                        <x-forms.textarea :label="__('messages.t_reason')" :placeholder="__('messages.t_report_user_reason_placeholder')" model="reason" icon="message-question" />
                    </div>
