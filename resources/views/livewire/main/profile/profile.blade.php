<div class="w-full" x-data="window.vjOXhkLsunWIxQy" x-init="initialize()" x-cloak>
    <div class="grid grid-cols-12 md:gap-x-6 gap-y-6">
                    ...
                    <div class="flex">
                        ...
                        <div class="ltr:ml-3 rtl:mr-3">
                            ...
                            <div class="mt-2 text-sm text-amber-700">
                                <span
                                    class="text-xs font-medium mb-3 block">{{ __('messages.t_this_user_is_not_available_right_now_msg', ['date' => format_date($user->availability->expected_available_date, config('carbon-formats.F_j,_Y'))]) }}</span>
        ...
        <div class="col-span-12 lg:col-span-4">
            ...
            {{-- Profile header --}}
            <div
                class="bg-white dark:bg-zinc-800 border border-gray-100 dark:border-zinc-700 rounded-xl shadow-sm overflow-hidden mb-6">
                <div class="md:flex">
                    <div class="w-full p-2 py-10">
                        ...
                        <div class="flex justify-center">
                            <div class="relative">
                                ...
                                {{-- User avatar --}}
                                <img src="{{ placeholder_img() }}" data-src="{{ src($user->avatar_id) }}"
                                    alt="{{ $user->username }}" class="lazy rounded-full w-20 h-20 object-cover">
                        ...
                        <div class="flex flex-col text-center mt-3 mb-4">
                            <span
                                class="text-md font-extrabold text-gray-800 dark:text-gray-100 flex items-center justify-center">
                                {{ $user->username }}
                                @if ($user->status === 'verified')
                                    <img data-tooltip-target="tooltip-account-verified-{{ $user->id }}"
                                        class="ltr:ml-0.5 rtl:mr-0.5 h-4 w-4 -mt-0.5"
                                        src="{{ url('img/auth/verified-badge.svg') }}"
                                        alt="{{ __('messages.t_account_verified') }}">
                                    <div id="tooltip-account-verified-{{ $user->id }}" role="tooltip"
                        ...
                        <div class="px-14 mt-8">
                            ...
                            @auth
                                @if (auth()->id() !== $user->id)
                                    <button id="modal-report-button" type="button"
                                        class="h-12 bg-gray-200 dark:bg-zinc-700 w-full text-black dark:text-zinc-300 text-sm font-medium rounded dark:hover:bg-zinc-600 hover:shadow hover:bg-gray-300 mb-2">{{ __('messages.t_report') }}</button>
                                @endif
                            @endauth
                    ...
                    <dl class="sm:divide-y sm:divide-gray-100 dark:divide-zinc-700">
                        ...
                        @if ($user->country)
                            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <dd
                                    class="mt-1 text-sm text-gray-900 dark:text-gray-200 sm:mt-0 sm:col-span-2 flex items-center">
                                    <img src="{{ placeholder_img() }}"
                                        data-src="{{ url('img/flags/rounded/' . strtolower($user->country?->code) . '.svg') }}"
                                        alt="{{ $user->country?->name }}" class="lazy h-5 w-5 ltr:mr-2 rtl:ml-2">
                ...
                <div
                    ...
                    <div class="border-t-2 border-gray-100 dark:border-zinc-700 px-4 py-5 sm:p-0">
                        <div class="px-6 py-5">
                                    ...
                                    @if ($project->status === 'active')
                                        <figure>
                                            <a href="{{ url('profile/' . $project->user->username . '/portfolio/' . $project->slug) }}"
                                                target="_blank">
                                                <img src="{{ placeholder_img() }}"
                                                    data-src="{{ src($project?->thumbnail_id) }}"
                                                    alt="{{ $project->title }}"
                                                    class="lazy rounded-md hover:opacity-75 object-cover" />
                                            </a>
                                        </figure>
                                    @endif
                                @endforeach
                            ...
                            <a href="{{ url('profile/' . $user->username . '/portfolio') }}" target="_blank"
                                class="block mt-8 text-center text-xs font-bold text-primary-600 hover:underline">{{ __('messages.t_view_my_porfolio') }}</a>
                        ...
                        @foreach ($user->skills as $skill)
                            <a href="{{ url('hire', $skill->slug) }}" class="block mb-1 px-6 py-5">
                                <div class="flex justify-between mb-1">
                                    ...
                                    <span
                                        class="text-xs font-normal text-gray-600 dark:text-gray-400">{{ __('messages.t_' . $skill->experience) }}</span>
                                </div>
                    ...
                    <div class="border-t-2 border-gray-100 dark:border-zinc-700 px-4 py-5 sm:p-0">
                        <div class="px-6 py-5 space-x-2 space-y-4 text-center rtl:space-x-reverse">
                            ...
                            @if ($user->accounts->facebook_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->facebook_profile)) }}"
                                    target="_blank"
                            ...
                            @if ($user->accounts->twitter_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->twitter_profile)) }}"
                                    target="_blank"
                                    class="text-white bg-[#00acee] hover:bg-[#00acee]/90 focus:outline-none rounded-full text-center inline-flex items-center h-10 w-10 justify-center">
                            ...
                            @if ($user->accounts->dribbble_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->dribbble_profile)) }}"
                                    target="_blank"
                                    class="text-white bg-[#ea4c89] hover:bg-[#ea4c89]/90 focus:outline-none rounded-full text-center inline-flex items-center h-10 w-10 justify-center">
                            ...
                            @if ($user->accounts->stackoverflow_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->stackoverflow_profile)) }}"
                                    target="_blank"
                                    class="text-white bg-[#ef8236] hover:bg-[#ef8236]/90 focus:outline-none rounded-full text-center inline-flex items-center h-10 w-10 justify-center">
                            ...
                            @if ($user->accounts->github_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->github_profile)) }}"
                                    target="_blank"
                                    class="text-white bg-[#171515] hover:bg-[#171515]/90 focus:outline-none rounded-full text-center inline-flex items-center h-10 w-10 justify-center">
                            ...
                            @if ($user->accounts->youtube_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->youtube_profile)) }}"
                                    target="_blank"
                                    class="text-white bg-[#FF0000] hover:bg-[#FF0000]/90 focus:outline-none rounded-full text-center inline-flex items-center h-10 w-10 justify-center">
                            ...
                            @if ($user->accounts->vimeo_profile)
                                <a href="{{ url('redirect?to=' . safeEncrypt($user->accounts->vimeo_profile)) }}"
                                    target="_blank"
                                    class="text-white bg-[#86c9ef] hover:bg-[#86c9ef]/90 focus:outline-none rounded-full text-center inline-flex items-center h-10 w-10 justify-center">
                                ...
                                @foreach ($user->languages as $lang)
                                    <li class="py-4 px-6">
                                        <div class="flex items-center space-x-4 rtl:space-x-reverse">
                                            ...
                                            <div>
                                                <span
                                                    class="text-xs text-gray-400 dark:text-gray-300 font-medium">{{ __('messages.t_' . $lang->level) }}</span>
                                            </div>
                                        </div>
                                    </li>
                                @endforeach
        ...
        <div class="col-span-12 lg:col-span-8">
                        ...
                        <div class="col-span-12">
                            <div class="py-14 px-6 text-center text-sm sm:px-14 border-dashed border-2">
                                ...
                                <p class="mt-2 text-gray-500">
                                    {{ __('messages.t_we_couldnt_find_anthing_search_term') }}
                                </p>
                                @if (auth()->user()?->username == request()->username && auth()->user()?->account_type == 'seller')
                                    <div class="flex justify-center mt-4">
                                        <a href="/create" class="btn-purple px-7">Create Gigs</a>
                                    </div>
                                @endif
                            </div>
                        </div>
            ...
            <div
                ...
                @forelse ($reviews as $review)
                    ...
                    <div class="col-span-12">
                        <div
                            class="bg-white dark:bg-zinc-700 dark:border-zinc-600 relative block p-8 overflow-hidden border-t border-gray-100 rounded-lg mb-3">
                ...
                @empty
                    <div class="col-span-12">
                        <div class="py-3 px-6 text-center text-sm sm:px-4 border-dashed border-2">
                            <svg class="mx-auto h-6 w-6 text-gray-400" xmlns="http://www.w3.org/2000/svg"
                                fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                            </svg>
                            <p class="mt-4 font-semibold text-gray-900">{{ 'No reviews found' }}</p>
    ...
    @if (auth()->check() && auth()->id() !== $user->id)
        <x-forms.modal id="modal-report-container" target="modal-report-button" uid="modal_{{ uid() }}"
            ...
            <x-slot name="content">
                <div class="grid grid-cols-12 md:gap-x-6 gap-y-6">
                    ...
                    {{-- Message --}}
                    <div class="col-span-12">
                        <x-forms.textarea :label="__('messages.t_reason')" :placeholder="__('messages.t_report_user_reason_placeholder')" model="reason"
