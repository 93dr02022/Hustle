<div class="gig-card" x-data="window._{{ $gig->uid }}" dir="{{ config()->get('direction') }}">
    <div class="overflow-hidden bg-white rounded-md shadow-sm dark:bg-zinc-800 ring-1 ring-white dark:ring-zinc-800">
        ...
        <a href="{{ url('service', $gig->slug) }}"
            ...
            <span
                class="absolute z-[1] top-0 left-0 text-xs font-normal bg-black/70 backdrop-blur-lg py-[3px] px-2.5 truncate max-w-[80px] m-1.5 text-white rounded">
                {{ $gig->state_name }}</span>
            <img class="object-fill w-full max-h-52 lazy h-52" width="200" src="{{ placeholder_img() }}"
        ...
        <div class="px-4 pb-2 mt-2.5">
            ...
            @if ($profile_visible)
                <div class="flex items-center justify-between w-full mb-4">
                    <a href="{{ url('profile', $gig->username) }}" target="_blank" class="flex-shrink-0 block group">
                        <div class="flex items-center">
                            <span class="relative inline-block">
                                <img class="object-cover w-6 h-6 rounded-full lazy" src="{{ placeholder_img() }}"
                                    data-src="{{ src($gig->avatar_id) }}" alt="{{ $gig->username }}">
                            </span>
        ...
        <div
            ...
            <div class="flex items-center">
                ...
                <div id="tooltip-add-to-favorites-{{ $gig->uid }}" role="tooltip"
                    class="absolute z-10 invisible inline-block px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                    @if ($favorite)
