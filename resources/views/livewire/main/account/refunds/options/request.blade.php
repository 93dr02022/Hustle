                    ...
                    <div class="py-6 px-4 sm:p-6 lg:pb-8">
                        ...
                        <div class="mb-14">
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">
                                {{ __('messages.t_request_refund_subtitle') }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-12 md:gap-x-8 gap-y-8 mb-6">
                                ...
                                <div
                                    ...
                                    <div class="py-6 px-4 sm:px-6 lg:grid lg:grid-cols-12 lg:gap-x-8 lg:p-8">
                                        <div class="sm:flex lg:col-span-7">
                                            <div
                                                class="flex-shrink-0 w-full aspect-w-1 aspect-h-1 rounded-lg overflow-hidden sm:aspect-none sm:w-28 sm:h-28">
                                                <img src="{{ placeholder_img() }}"
                                                    data-src="{{ src($item->gig->image_thumb_id) }}"
                                                    alt="{{ $item->gig->title }}"
                                                    class="lazy w-full h-full object-center object-cover sm:w-full sm:h-full">
                                            </div>

                                            <div class="mt-6 sm:mt-0 sm:ltr:ml-6 sm:rtl:mr-6">
                                                <h3
                                                    class="text-base font-medium text-gray-900 dark:text-gray-100 dark:hover:text-primary-600 hover:text-primary-600">
                                                            ...
                                                            @foreach ($item->upgrades as $upgrade)
                                                                <div class="relative flex items-start">
                                                                    <div class="flex items-center h-5">
                                                                        <input type="checkbox"
                                                                            class="h-4 w-4 text-gray-300 border-gray-200 dark:border-zinc-600 border-2 rounded-sm cursor-not-allowed pointer-events-none"
                                                                            checked disabled>
                                                                    </div>
                                    ...
                                    <div
                                        class="border-t-2 border-dashed border-gray-200 dark:border-zinc-600 py-6 px-4 sm:px-6 lg:p-8">
                                        <h4 class="sr-only">Status</h4>
                                        <p class="text-sm font-medium text-gray-900 dark:text-gray-300">
                                            {{ __('messages.t_expected_delivery_date_on_date', ['date' => format_date($item->expected_delivery_date, config('carbon-formats.F_j,_Y_h_:_i_A'))]) }}
                                        </p>
                                        <div class="mt-6" aria-hidden="true">
                                            ...
                                            <div
                                                class="hidden sm:grid grid-cols-3 text-sm font-medium text-gray-600 mt-6">
                                                <div class="text-primary-600">{{ __('messages.t_order_placed') }}</div>
                                                <div
                                                    class="text-center {{ in_array($item->status, ['proceeded', 'delivered']) ? 'text-primary-600' : '' }}">
                                                    {{ __('messages.t_processing') }}</div>
                                                <div
                                                    class="ltr:text-right rtl:text-left {{ $item->status === 'delivered' ? 'text-primary-600' : '' }}">
                                                    {{ __('messages.t_delivered') }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-span-12">
                                <x-forms.textarea label="{{ __('messages.t_reason') }}"
                                    placeholder="{{ __('messages.t_enter_refund_reason') }}" model="reason"
                                    :rows="18" icon="card-text-outline" />
                            </div>
