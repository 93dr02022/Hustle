<main class="w-full" x-data="window.aelkGHWmZHjwNJZ" x-init="initialize()" x-on:livewire-upload-start="uploadStart()"
    x-on:livewire-upload-finish="uploadFinish()" x-on:livewire-upload-error="uploadError()"
                    ...
                    <div class="py-6 px-4 sm:p-6 lg:pb-8">
                        ...
                        <div class="mb-14">
                            <h2 class="text-base leading-6 font-bold text-gray-900 dark:text-gray-100">
                                {{ __('messages.t_send_requirements') }}</h2>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">
                                {{ __('messages.t_send_requirements_subtitle') }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-12 md:gap-x-8 gap-y-8 mb-6">
                            ...
                            {{-- Submit files form --}}
                            <div
                                class="col-span-12 xl:col-span-7 grid items-end border-2 border-dashed dark:border-zinc-700 rounded-md {{ $item->requirements()->exists() ? 'relative' : '' }}">

                                {{-- Blury div to prevent resubmit again (Just in client side don't worry) --}}
                                @if ($item->requirements()->exists())
                                    <div
                                        class="absolute inset-6 backdrop-blur-sm w-full h-full top-0 ltr:left-0 rtl:right-0 bg-gray-100/60 dark:bg-black/25 z-30 rounded-bl-md">
                                    </div>
                                @endif
                                ...
                                <div class="grid grid-cols-12 md:gap-x-6 gap-y-6 px-6 py-8">
                                        ...
                                        <div class="flex flex-col space-y-4 overflow-y-auto">
                                            <div class="flex items-center">
                                                <div
                                                    ...
                                                    <div><span
                                                            class="px-4 py-2 rounded-lg italic inline-block bg-gray-100 dark:bg-zinc-600 text-gray-900 dark:text-gray-300 w-full">{{ __('messages.t_hey_before_i_start_working_on_order_msg') }}</span>
                                                    </div>
                                                </div>
                                                <img src="{{ placeholder_img() }}"
                                                    data-src="{{ src($item->owner->avatar_id) }}"
                                    ...
                                    @foreach ($item->gig->requirements as $key => $req)
                                        ...
                                        @if ($req->type === 'text')
                                            <div class="col-span-12">
                                                <label for="requirements-text-{{ $key }}-{{ $item->uid }}"
                                                ...
                                                <div class="mt-2 relative">
                                                    <textarea placeholder="{{ __('messages.t_type_ur_message_here') }}"
                                                        wire:model.defer="requirements.{{ $req->id }}.value" rows="8"
                                                        id="requirements-text-{{ $key }}-{{ $item->uid }}"
                                                        class="dark:bg-transparent block w-full text-xs rounded border-2 ltr:pr-10 rtl:pl-10 ltr:pl-3 rtl:pr-3 py-3 font-normal resize-none border-gray-200 dark:border-zinc-600 placeholder-gray-400 focus:ring-primary-600 focus:border-primary-600 dark:text-gray-50"></textarea>
                                                    <div
                                                            ...
                                                            @foreach ($req->options as $index => $option)
                                                                <div class="flex items-center">
                                                                    <div class="flex items-center h-5">
                                                                        <input
                                                                            id="requirements-choice-multiple-{{ $key }}-option-{{ $index }}"
                                                                            wire:model.defer="requirements.{{ $req->id }}.value.{{ $index }}"
                                                                            value="{{ $option->option }}"
                                                                            type="checkbox"
                                                                            class="focus:ring-primary-600 h-4 w-4 text-primary-600 border-gray-300 dark:border-zinc-500 dark:bg-zinc-600 d rounded-sm border-2">
                                                                    </div>
                                                                    <div class="ltr:ml-3 rtl:mr-3 text-xs">
                                                                        <label
                                                                            for="requirements-choice-multiple-{{ $key }}-option-{{ $index }}"
                                                                            class="font-medium text-gray-700 dark:text-gray-100 cursor-pointer">{{ $option->option }}</label>
                                                                    </div>
                                                                </div>
                                                            @endforeach
                                                        ...
                                                        <div class="mt-4 space-y-3">
                                                            ...
                                                            @foreach ($req->options as $index => $option)
                                                                <div class="flex items-center">
                                                                    <div class="flex items-center h-5">
                                                                        <input
                                                                            id="requirements-choice-not-multiple-{{ $key }}-option-{{ $index }}"
                                                                            wire:model.defer="requirements.{{ $req->id }}.value"
                                                                            value="{{ $option->option }}"
                                                                            type="radio"
                                                                            name="requirements-choice-not-multiple-{{ $key }}.options"
                                                                            class="focus:ring-primary-600 h-4 w-4 text-primary-600 border-gray-300 dark:border-zinc-500 dark:bg-zinc-600 d rounded-full border-2">
                                                                    </div>
                                                                    <div class="ltr:ml-3 rtl:mr-3 text-xs">
                                                                        <label
                                                                            for="requirements-choice-not-multiple-{{ $key }}-option-{{ $index }}"
                                                                            class="font-medium text-gray-700 dark:text-gray-100 cursor-pointer">{{ $option->option }}</label>
                                                                    </div>
                                                                </div>
                                                            @endforeach
                                    ...
                                    <div class="col-span-12">
                                        ...
                                        <div class="mt-2 relative">
                                            <input type="file" wire:model.defer="requirement_file"
                                ...
                                {{-- Footer actions --}}
                                <div
                                    class="px-4 py-3 bg-gray-50 dark:bg-zinc-700 ltr:text-right rtl:text-left sm:px-6 rounded-bl-md flex justify-end">
                            ...
                            <div
                                class="col-span-12 xl:col-span-5 border-2 border-dashed dark:border-zinc-700 rounded-md {{ $item->is_requirements_sent && $item->requirements()->count() ? 'grid items-end' : '' }}">
                                <div class="grid grid-cols-12 md:gap-x-6 gap-y-6 px-6 py-8">
                                    ...
                                    @if ($item->is_requirements_sent && $item->requirements()->count())
                                        <div class="col-span-12">
                                            <div
                                                class="bg-yellow-50 ltr:border-l-4 rtl:border-r-4 border-yellow-400 p-4">
                                                <div class="flex">
                                ...
                                @if ($item->is_requirements_sent && $item->requirements()->count())
                                    <div
                                        class="px-4 py-3 bg-gray-50 dark:bg-zinc-700 text-right sm:px-6 rounded-br-md flex justify-end">
    ...
    {{-- Uploading modal --}}
    <div id="uploading-modal" tabindex="-1"
                ...
                <div class="p-6 text-center">
                    ...
                    <div>
                        <div class="flex justify-between mb-1">
                            <span
                                class="text-sm font-medium text-primary-600 dark:text-white">{{ __('messages.t_uploading') }}</span>
                            <span class="text-sm font-medium text-primary-600 dark:text-white"
                                x-text="uploadText()"></span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2.5">
                            <div class="bg-primary-600 h-2.5 rounded-full" :style="{ width: uploadingProgress + '%' }">
                            </div>
                        </div>
                    </div>
