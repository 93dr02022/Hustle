                    ...
                    <div class="py-6 px-4 sm:p-6 lg:pb-8 h-[calc(100%-80px)]">
                        ...
                        <div class="w-full mb-6">
                            ...
                            <div class="mb-10 bg-white border border-gray-200 rounded-md dark:bg-zinc-700 dark:border-zinc-600" wire:key="orders-{{ $order->uid }}">
                                <div class="flex items-center p-4 border-b bg-gray-50 dark:bg-zinc-600 sm:rounded-t-md sm:p-6 sm:grid sm:grid-cols-4 sm:gap-x-6 dark:border-zinc-600">

                                    {{-- Order quick stats --}}
                                    <dl class="grid flex-1 grid-cols-2 text-sm gap-x-6 sm:col-span-3 sm:grid-cols-2 lg:col-span-2">
                                ...
                                <ul role="list" class="divide-y divide-gray-200 dark:divide-zinc-600">
                                    ...
                                    <li class="p-4 sm:p-6">
                                        <div class="flex items-center sm:items-start">
                                            ...
                                            {{-- Gig thumbnail --}}
                                            <div class="flex-shrink-0 hidden w-24 h-24 overflow-hidden bg-gray-200 rounded-lg dark:bg-zinc-600 md:block">
                                                <img src="{{ placeholder_img() }}" data-src="{{ src($item->gig->image_thumb_id) }}" alt="{{ $item->gig->title }}" class="object-cover object-center w-full h-full text-xs lazy">
                                            </div>
                                            ...
                                            <div class="flex-1 text-sm ltr:md:ml-6 rtl:md:mr-6">
                                                <div class="md:flex md:items-center md:justify-between md:space-x-4 rtl:space-x-reverse xl:pb-4">
                                                    <div>
                                                        ...
                                                        {{-- Item title --}}
                                                        <h1 class="mb-2 text-base font-bold text-gray-900 hover:text-primary-600 dark:text-white">
                                                            <a href="{{ url('account/orders/view-order', $order->uid) }}" target="_blank">{{ $item->gig->title }}</a>
                                                        </h1>
                                                        ...
                                                        <div class="inline-block md:!flex text-gray-500 dark:text-gray-300 text-xs space-y-4 md:space-y-0 md:space-x-6 lg:space-x-12 rtl:space-x-reverse">
                                                            ...
                                                            <span class="flex">
                                                                <div class="md:text-center">
                                                                    <p class="text-[10px] tracking-widest text-gray-400 dark:text-gray-300 uppercase">
                                                            ...
                                                            <span class="flex">
                                                                <div class="md:text-center">
                                                                    ...
                                                                    <div id="orders-{{ $order->uid }}-item-{{ $item->id }}-status-proceeded" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                                                    ...
                                                                    <div id="orders-{{ $order->uid }}-item-{{ $item->id }}-status-delivered" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                                                    ...
                                                                    <div id="orders-{{ $order->uid }}-item-{{ $item->id }}-status-canceled" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                                                    ...
                                                                    <div id="orders-{{ $order->uid }}-item-{{ $item->id }}-status-refunded" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                                    ...
                                                    <div class="flex mt-4 space-x-3 md:mt-0">
                                                        <button type="button" data-dropdown-toggle="orders-{{ $order->uid }}-item-{{ $item->id }}-actions-dropdown" data-dropdown-placement="left-start" class="inline-flex justify-center px-2 py-2 text-xs font-medium text-gray-700 bg-white border border-gray-200 rounded-full shadow-sm dark:border-zinc-700 dark:text-gray-300 dark:bg-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-900 focus:outline-none focus:ring-0">
                                                        ...
                                                        <div id="orders-{{ $order->uid }}-item-{{ $item->id }}-actions-dropdown" class="z-10 hidden w-48 bg-white divide-y divide-gray-100 rounded shadow dark:bg-zinc-800">
                                                            <ul class="text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefault">
                                                                ...
                                                                {{-- Contact seller --}}
                                                                <li>
                                                                    <a href="{{ url('messages/new', $item->owner?->username) }}" target="_blank" class="flex items-center px-4 py-2 rounded-t hover:bg-gray-100 dark:hover:bg-zinc-700">
                                                                        <svg class="w-5 h-5 text-gray-500 ltr:mr-3 rtl:ml-3" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                                                                ...
                                                                <li>
                                                                    <a href="{{ url('account/orders/files?orderId=' . $order->uid . '&itemId=' . $item->uid) }}" class="flex items-center px-4 py-2 hover:bg-gray-100 dark:hover:bg-zinc-700">
                                                                        <svg class="w-5 h-5 text-gray-500 ltr:mr-3 rtl:ml-3" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                                ...
                                                                <li>
                                                                    <button type="button" x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_cancel_service') }}') ? $wire.cancel('{{ $item->uid }}') : ''" wire:loading.attr="disabled" wire:target="cancel('{{ $item->uid }}')" class="flex items-center w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-zinc-700">

                                                                        {{-- Loading indicator --}}
                                                                        <div wire:loading wire:target="cancel('{{ $item->uid }}')">
                                                                            <svg role="status" class="inline w-5 h-5 text-gray-500 ltr:mr-3 rtl:ml-3 animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                        ...
                                                                        <div wire:loading.remove wire:target="cancel('{{ $item->uid }}')">
                                                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-500 ltr:mr-3 rtl:ml-3 group-hover:text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                                ...
                                                                <li>
                                                                    <a href="{{ url('account/orders/requirements?order=' . $order->uid . '&item=' . $item->uid) }}" class="flex items-center px-4 py-2 rounded-b hover:bg-gray-100 dark:hover:bg-zinc-700">
                                                                        <svg class="w-5 h-5 text-gray-500 ltr:mr-3 rtl:ml-3" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                                ...
                                                                <li>
                                                                    <a href="{{ url('account/reviews/create/' . $item->uid) }}" class="flex items-center px-4 py-2 rounded-b hover:bg-gray-100 dark:hover:bg-zinc-700">
                                                                        <svg class="w-5 h-5 text-gray-500 ltr:mr-3 rtl:ml-3" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    ...
                                                    <fieldset class="space-y-5">
                                                        ...
                                                        <div class="relative flex items-start">
                                                            <div class="flex items-center h-5">
                                                                <input type="checkbox" class="w-4 h-4 text-gray-300 border-2 border-gray-200 rounded-sm cursor-not-allowed pointer-events-none dark:text-gray-400 dark:border-zinc-600 dark:disabled:bg-zinc-600" checked disabled>
                                                            </div>
                                                            <div class="ltr:ml-3 rtl:mr-3 text-sm mt-[-3px]">
                                                                <label class="text-xs font-medium text-gray-500 dark:text-gray-100">
                                                                    {{ $upgrade->title }}
                                                                </label>
                                                                ...
                                                                <div class="flex mt-1 text-sm">
                                                                    ...
                                                                    <p class="text-xs text-gray-400 border-gray-200 ltr:ml-4 rtl:mr-4 ltr:pl-4 rtl:pr-4 ltr:border-l rtl:border-r dark:border-zinc-600 dark:text-gray-300">
                                        ...
                                        <div class="mt-6 sm:flex sm:justify-between">
                                            ...
                                            <div class="flex items-center">
                                                ...
                                                <p class="text-xs font-medium ltr:ml-2 rtl:mr-2 text-amber-600">
                                            ...
                                            {{-- Footer actions --}}
                                            <div class="flex items-center pt-4 mt-6 space-x-4 text-sm font-medium border-t border-gray-200 divide-x divide-gray-200 dark:divide-zinc-600 rtl:divide-x-reverse sm:mt-0 sm:ltr:ml-4 sm:rtl:mr-4 sm:border-none sm:pt-0 rtl:space-x-reverse">
                            ...
                            <div class="mb-10 bg-white border border-gray-200 rounded-md dark:bg-zinc-700 dark:border-zinc-600" wire:key="orders-{{ $order->uid }}">
                                ...
                                <div role="list" class="p-4 divide-y divide-gray-200 bg-blue-50 dark:bg-slate-700 dark:divide-slate-600 sm:p-6">
                                    {{-- Item quick stats --}}
                                    <div class="grid grid-cols-2 mb-5 sm:grid-cols-3 md:grid-cols-4 gap-x-4 gap-y-4">
                                    ...
                                    {{-- Footer actions --}}
                                    <div class="pt-4 space-x-4 border-t border-gray-200 divide-x divide-gray-200 dark:divide-zinc-600 sm:mt-0 sm:border-none sm:pt-0">
