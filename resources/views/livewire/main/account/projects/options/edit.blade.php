            ...
            <div class="lg:col-span-9 lg:ltr:ml-8 lg:rtl:mr-8">
                ...
                <div class="w-full">
                    <div id="app">
                        ...
                        {{-- Form --}}
                        <edit-project _project='@json($project)' categories="{{ json_encode($categories) }}" translations="{{ json_encode($translations) }}"
                            selected_skills="{{ json_encode($skills) }}" plans="{{ json_encode($plans) }}" _settings="{{ $settings }}" currency_symbol="{{ $currency_symbol }}"
                            is_recaptcha="{{ settings('security')->is_recaptcha }}" recaptcha_site_key="{{ config('recaptcha.site_key') }}" />
