<div class="w-full">
    ...
    <div class="lg:grid lg:grid-cols-12">
        ...
        <div class="lg:col-span-9 lg:ltr:ml-8 lg:rtl:mr-8">
            ...
            <div class="w-full">
                ...
                <div class="w-full mb-6">
                    <div
                        class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-y-6 gap-x-3 h-full">
                        ...
                        <div>
                            <div
                                ...
                                <div
                                    class="w-10 h-10 rounded-full flex items-center justify-center border border-slate-200 bg-slate-100 dark:bg-zinc-700 dark:border-transparent">
                                    <svg class="h-5 w-5 text-slate-400 dark:text-zinc-400" stroke="currentColor"
                                        fill="currentColor" stroke-width="0" version="1.2" baseProfile="tiny"
                                ...
                                <span
                                    class="font-medium text-slate-400 text-[11px] uppercase dark:text-zinc-300 tracking-[2px] whitespace-nowrap">@lang('messages.t_status')</span>
                        ...
                        <div>
                            <div
                                ...
                                <span
                                    class="font-medium text-slate-400 text-[11px] uppercase dark:text-zinc-300 tracking-[2px] whitespace-nowrap">@lang('messages.t_delivery_time')</span>
                                <div class="text-sm font-semibold tracking-wide dark:text-white text-zinc-700">
                        ...
                        <div>
                            <div
                                ...
                                <span
                                    class="font-medium text-slate-400 text-[11px] uppercase dark:text-zinc-300 tracking-[2px] whitespace-nowrap">@lang('messages.t_in_progress')</span>
                                <div class="text-sm font-semibold tracking-wide dark:text-white text-zinc-700">
                        ...
                        <div>
                            <div
                                ...
                                <span
                                    class="font-medium text-slate-400 text-[11px] uppercase dark:text-zinc-300 tracking-[2px] whitespace-nowrap">@lang('messages.t_paid_amount')</span>
                                <div class="text-sm font-semibold tracking-wide dark:text-white text-zinc-700">
                        ...
                        <div>
                            <div
                                ...
                                <span
                                    class="font-medium text-slate-400 text-[11px] uppercase dark:text-zinc-300 tracking-[2px] whitespace-nowrap">@lang('messages.t_project_budget')</span>
                                <div class="text-sm font-semibold tracking-wide dark:text-white text-zinc-700">
                ...
                <div class="w-full">
                    ...
                    {{-- Table --}}
                    <div
                        class="mt-8 overflow-x-auto overflow-y-hidden scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 dark:scrollbar-thumb-zinc-800 dark:scrollbar-track-zinc-600">
                            ...
                            <thead>
                                @forelse ($payments as $p)
                                    <tr class="intro-x shadow-sm bg-white dark:bg-zinc-800 rounded-md h-16"
                                        wire:key="employer-dashboard-project-milestones-{{ $p->uid }}">

                                        {{-- Date --}}
                                        <td
                                            class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md rtl:text-right">
                                            <div
                                                class="text-gray-600 dark:text-gray-100 text-[13px] font-normal whitespace-nowrap">
                                        ...
                                        {{-- Description --}}
                                        <td
                                            class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md w-48 rtl:text-right">
                                            <div
                                                class="text-gray-500 leading-relaxed dark:text-gray-100 text-[13px] font-normal break-words flex-none truncate w-48 overflow-hidden block">
                                                {{ $p->description }}
                                            </div>
                                        </td>
                                        ...
                                        <td
                                            ...
                                            <div
                                                ...
                                                <x-forms.tooltip id="tooltip-actions-details-{{ $p->uid }}"
                                                    :text="__('messages.t_details')" />

                                                {{-- Pay --}}
                                                @if ($p->status === 'request')
                                                    <button wire:click="confirmPay('{{ $p->uid }}')"
                                                        type="button"
                                                        class="inline-flex justify-center items-center border font-semibold focus:outline-none w-8 h-8 text-sm rounded border-gray-300 bg-white text-gray-600 shadow-sm hover:text-gray-800 hover:bg-gray-100 hover:border-gray-300 hover:shadow focus:ring focus:ring-gray-500 focus:ring-opacity-25 active:bg-white active:border-white active:shadow-none dark:bg-zinc-700 dark:border-transparent dark:text-zinc-200 dark:hover:bg-zinc-600"
                                                    ...
                                                    <x-forms.tooltip id="tooltip-actions-pay-{{ $p->uid }}"
                                                        :text="__('messages.t_deposit_funds')" />
                                                @endif

                                                {{-- Release --}}
                                                @if ($p->status === 'funded')
                                                    <button wire:click="confirmRelease('{{ $p->uid }}')"
                                                        type="button"
                                                    ...
                                                    <x-forms.tooltip id="tooltip-actions-release-{{ $p->uid }}"
                                                        :text="__('messages.t_release')" />
                                                @endif
                                ...
                                @if ($payments->count())
                                    <tfoot>
                                        <tr class="bg-slate-200 dark:bg-zinc-600 intro-x rounded-md h-16">
                                            <th colspan="3"
                                                class="text-center py-3 px-5 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md">
                                            </th>
                                            <td
        ...
        {{-- Awarded bid modal --}}
        <x-forms.modal id="modal-awarded-bid-container-{{ $project->uid }}"
            target="modal-awarded-bid-button-{{ $project->uid }}" uid="modal_awarded_bid_{{ $project->uid }}"
                            ...
                            <div class="flex items-center space-x-4 rtl:space-x-reverse">
                                ...
                                <a href="{{ url('profile', $project->awarded_bid->user->username) }}" class="block">
                                    <img class="rounded-full h-12 w-12 object-cover object-center lazy"
                                        src="{{ placeholder_img() }}"
                                        data-src="{{ src($project->awarded_bid->user->avatar_id) }}"
                                ...
                                <div class="space-y-0.5">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        @if ($project->awarded_bid->user->fullname)
                                            <span
                                                class="font-medium text-zinc-900 text-sm hover:text-black dark:text-zinc-200 dark:hover:text-white ltr:pr-1 rtl:pl-1">
                                    ...
                                    <div
                                        ...
                                        @if ($project->awarded_bid->user->country)
                                            <p class="flex items-center space-x-1 rtl:space-x-reverse">
                                                <img class="h-4 ltr:pr-0.5 rtl:pl-0.5 -mt-0.5 lazy"
                                                    src="{{ placeholder_img() }}"
                                                    data-src="{{ countryFlag($project->awarded_bid->user->country->code) }}"
                                                    alt="{{ $project->awarded_bid->user->country->name }}">
                                        ...
                                        <p class="flex shrink-0 items-center space-x-1 rtl:space-x-reverse">
                                            <svg aria-hidden="true"
                                                class="w-4 h-4 {{ $project->awarded_bid->user->rating() == 0 ? 'text-gray-400' : 'text-amber-500' }} -mt-0.5"
                                                fill="currentColor" viewBox="0 0 20 20"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
        ...
        {{-- Create milestone --}}
        @if (in_array($project->status, ['active', 'under_development', 'pending_final_review']))
            <x-forms.modal id="modal-create-milestone-container-{{ $project->uid }}"
                target="modal-create-milestone-button-{{ $project->uid }}"
                uid="modal_create_milestone_{{ $project->uid }}" placement="center-center" size="max-w-md">
                ...
                <x-slot name="content">
                    <div class="grid grid-cols-1 gap-y-6">
                        ...
                        {{-- Amount --}}
                        <div>
                            <x-forms.text-input :label="__('messages.t_amount')" placeholder="0.00" model="milestone_amount"
                        ...
                        {{-- Description --}}
                        <div>
                            <x-forms.textarea :label="__('messages.t_description')" :placeholder="__('messages.t_enter_milestone_payment_description')" model="milestone_description"
                                :rows="6"
