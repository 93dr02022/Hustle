<main class="w-full" x-data>
    <div class="px-4 sm:px-6 lg:px-8">
        ...
        @if (session()->has('success'))
            <div
                class="p-4 relative flex bg-emerald-50 dark:bg-emerald-500 text-emerald-500 dark:text-emerald-50 text-xs font-semibold rounded-lg mb-4">
        ...
        @if (session()->has('warning'))
            <div
                class="p-4 relative flex bg-amber-50 dark:bg-amber-500 text-amber-500 dark:text-amber-50 text-xs font-semibold rounded-lg mb-4">
                <div class="flex items-center">
                    ...
                    <div class="w-full">
                        ...
                        <div class="mb-14 py-6 px-4 sm:p-4">
                            <h2 class="text-base leading-6 font-bold text-gray-900 dark:text-gray-100">
                                {{ __('messages.t_deposit_history') }}</h2>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">
                                {{ __('messages.t_deposit_history_subtitle') }}</p>
                        </div>
                                ...
                                <thead class="bg-gray-100 dark:bg-zinc-700">
                                    <tr tabindex="0"
                                        ...
                                        <th
                                            ...
                                            {{ __('messages.t_payment_method') }}</th>
                                        <th
                                            class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center">
                                        ...
                                        <tr class="focus:outline-none text-sm leading-none text-gray-800 bg-white dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 border-b border-t border-gray-100 dark:border-zinc-700/40"
                                            ...
                                            <td class="ltr:pl-4 rtl:pr-4">
                                                ...
                                                <div class="grid items-center text-center justify-center"
                                                    data-tooltip-target="tooltip-year-date-{{ $t->id }}">
                                                    <span
                                                        class="text-base font-medium text-gray-500 dark:text-gray-200 mb-2">
                                                        {{ $formatted_date->format('d') }}
                                                    </span>
                                                    <span
                                                        class="text-[10px] text-gray-400 dark:text-gray-300 uppercase tracking-widest">
                                                        {{ $formatted_date->format('F') }}
                                                    </span>
                                                </div>

                                                <div id="tooltip-year-date-{{ $t->id }}" role="tooltip"
                                                    class="inline-block absolute invisible z-10 py-2 px-3 text-xs font-semibold text-white bg-gray-900 rounded-md shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                                    ...
                                                    <div class="flex items-center">
                                                        ...
                                                        @if (settings($t->payment_method)->logo)
                                                            <img src="{{ placeholder_img() }}"
                                                                data-src="{{ src(settings($t->payment_method)->logo_id) }}"
                                                                alt="{{ settings($t->payment_method)->name }}"
                                                        ...
                                                        <div class="ml-3 rtl:mr-3">
                                                            <div class="flex items-center">
                                                                <div
                                                                    class="text-gray-700 dark:text-gray-100 font-bold text-sm">
                                                                    {{ settings($t->payment_method)->name }}
                                                                </div>
                                                            </div>
                                                            <span
                                                                class="text-gray-500 dark:text-gray-300 font-medium text-xs mt-2 block">{{ $t->transaction_id }}</span>
                                                        </div>
                                            ...
                                            {{-- Amount --}}
                                            <td
                                                class="text-center font-black text-gray-600 dark:text-white text-[13px]">
                                                {{ _price($t->amount_net) }}
                                            </td>
                                            ...
                                            <td class="text-center px-4">
                                                @switch($t->status)
                                                    ...
                                                    @case('paid')
                                                        <div
                                                            class="rounded-sm px-1 py-2 bg-green-50 text-green-400 text-xs font-semibold">
                                                            {{ __('messages.t_paid') }}
                                                        </div>
                                                    @break
                                                    ...
                                                    @case('pending')
                                                        <div
                                                            class="rounded-sm px-1 py-2 bg-amber-50 text-amber-400 text-xs font-semibold">
                                                    ...
                                                    @case('rejected')
                                                        <div
                                                            class="rounded-sm px-1 py-2 bg-red-50 text-red-400 text-xs font-semibold">
