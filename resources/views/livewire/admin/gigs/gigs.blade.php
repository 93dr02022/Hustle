        ...
        <div class="sm:flex items-center justify-between">
            ...
            <div class="space-x-3 rtl:space-x-reverse">
                ...
                <a href="{{ admin_url('gigs/trash') }}"
                    ...
                    <svg class="w-4.5 h-4.5 -mt-0.5" stroke="currentColor" fill="currentColor" stroke-width="0"
            ...
            <thead class="bg-gray-200">
                <tr tabindex="0"
                    ...
                    <th
                        ...
                        {{ __('messages.t_gig') }}</th>
                    <th
                        class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center">
                        {{ __('messages.t_price') }}</th>
                    <th
                        class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center">
                        {{ __('messages.t_visits') }}</th>
                    <th
                        class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center">
                        {{ __('messages.t_sales') }}</th>
                    <th
                        class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center">
                        {{ __('messages.t_in_queue') }}</th>
                    <th
                        class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center">
                        {{ __('messages.t_rating') }}</th>
                    <th
                        class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center">
            ...
            <tbody class="w-full">
                ...
                @foreach ($gigs as $gig)
                    <tr class="focus:outline-none text-sm leading-none text-gray-800 bg-white dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 border-b border-t border-gray-100 dark:border-zinc-700/40"
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ltr:pl-4 rtl:pr-4">
                                    <a href="{{ url('service', $gig->slug) }}" target="_blank"
                                        class="font-medium text-sm hover:text-primary-600 truncate pb-1.5 block max-w-xs">{{ $gig->title }}</a>
                                    <div class="flex items-center text-[11px] font-normal text-gray-400">
                                        <a href="{{ url('categories', $gig->category->slug) }}"
                                            class="hover:text-gray-600">{{ $gig->category->name }}</a>
                                        <i class="mdi mdi-chevron-right text-[10px] mx-2"></i>
                                        <a href="{{ url('categories/' . $gig->category->slug . '/' . $gig->subcategory->name) }}"
                                            class="hover:text-gray-600">{{ $gig->subcategory->name }}</a>
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="text-center">
                            <span
                                class="text-xs text-gray-500 font-medium">{{ format_date($gig->created_at, 'ago') }}</span>
                        </td>
                        ...
                        <td class="text-center">
                            <div class="relative inline-block text-left">
                                <div>
                                    <button data-dropdown-toggle="table-options-dropdown-{{ $gig->id }}"
                                        type="button"
                                        class="inline-flex justify-center items-center rounded-full h-8 w-8 bg-white dark:bg-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-800 focus:outline-none focus:ring-0"
                                ...
                                <div id="table-options-dropdown-{{ $gig->id }}"
                                    class="hidden z-40 origin-top-right absolute right-0 mt-2 w-44 rounded-md shadow-lg bg-white dark:bg-zinc-800 ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 dark:divide-zinc-700 focus:outline-none"
                                    ...
                                    <div class="py-1" role="none">
                                        ...
                                        @if ($gig->status === 'pending')
                                            <button wire:key="option-publish-{{ $gig->id }}"
                                                x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_publish_this_gig') }}') ? $wire.publish('{{ $gig->id }}') : ''"
                                                wire:loading.attr="disabled"
                                                wire:target="publish('{{ $gig->id }}')" type="button"
                                                class="text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group flex items-center px-4 py-2 text-sm"
                                                ...
                                                <span
                                                    class="text-xs font-medium">{{ __('messages.t_publish_gig') }}</span>

                                            </button>
                                        @endif

                                        {{-- Edit --}}
                                        <a href="{{ admin_url('gigs/edit/' . $gig->uid . '?step=overview') }}"
                                            class="text-gray-800 group flex items-center px-4 py-2 text-sm"
                                            role="menuitem" tabindex="-1">
                                            <svg class="ltr:mr-3 rtl:ml-3 h-5 w-5 text-gray-400 group-hover:text-gray-500"
                                        ...
                                        {{-- Analytics --}}
                                        <a href="{{ admin_url('gigs/analytics/' . $gig->uid) }}"
                                            class="text-gray-800 group flex items-center px-4 py-2 text-sm"
                                            role="menuitem" tabindex="-1">
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                class="ltr:mr-3 rtl:ml-3 h-5 w-5 text-gray-400 group-hover:text-gray-500"
                                            ...
                                            <span
                                                class="text-xs font-medium">{{ __('messages.t_gig_analytics') }}</span>
                                        </a>

                                        {{-- Delete gig --}}
                                        <button wire:key="option-delete-{{ $gig->id }}"
                                            x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_delete_this') }}') ? $wire.delete('{{ $gig->id }}') : ''"
                                            wire:loading.attr="disabled" wire:target="delete('{{ $gig->id }}')"
                                            type="button"
                                            class="text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group flex items-center px-4 py-2 text-sm"
                                            role="menuitem" tabindex="-1">
                                            ...
                                            <span
                                                class="text-xs font-medium">{{ __('messages.t_move_to_trash') }}</span>
