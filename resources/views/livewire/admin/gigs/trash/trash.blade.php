        ...
        <div class="overflow-x-auto min-w-full bg-white">
            <table class="min-w-full text-sm align-middle whitespace-nowrap">
                ...
                <thead>
                    <tr>
                        <th
                            class="px-7 py-3 text-slate-600 bg-slate-100 font-bold text-[10px] uppercase tracking-widest ltr:text-left rtl:text-right">
                            @lang('messages.t_gig')
                        </th>
                        <th
                            class="p-3 text-slate-600 bg-slate-100 font-bold text-[10px] uppercase tracking-widest text-center">
                    ...
                    @foreach ($gigs as $gig)
                        <tr wire:key="trashed-gigs-{{ $gig->uid }}">
                            ...
                            <td class="p-3 text-center">
                                <span
                                    class="text-gray-500 font-semibold text-[13px]">{{ format_date($gig->deleted_at) }}</span>
                            </td>
                            ...
                            <td class="p-3 text-center">
                                <div class="space-x-4 rtl:space-x-reverse">
                                    ...
                                    {{-- Restore gig --}}
                                    <button type="button" wire:click="confirmRestore('{{ $gig->id }}')"
                                        data-tooltip-target="tooltip-action-restore-{{ $gig->id }}"
                                    ...
                                    <div id="tooltip-action-restore-{{ $gig->id }}" role="tooltip"
                                    ...
                                    {{-- Permanently delete --}}
                                    <button type="button" wire:click="confirmDelete('{{ $gig->id }}')"
                                        data-tooltip-target="tooltip-action-delete-{{ $gig->id }}"
                                        class="inline-flex justify-center items-center border font-semibold focus:outline-none w-8 h-8 leading-5 text-sm rounded border-gray-300 bg-white text-gray-800 shadow-sm hover:text-gray-800 hover:bg-gray-100 hover:border-gray-300 hover:shadow focus:ring focus:ring-gray-500 focus:ring-opacity-25 active:bg-white active:border-white active:shadow-none">
                                        <svg class="w-4 h-4" stroke="currentColor" fill="currentColor" stroke-width="0"
                                            viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path
                                    ...
                                    <div id="tooltip-action-delete-{{ $gig->id }}" role="tooltip"
                                        class="inline-block absolute invisible z-10 py-2 px-3 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
