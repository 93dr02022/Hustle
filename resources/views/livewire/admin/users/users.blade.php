<div class="w-full">
    ...
    <div class="px-4 md:px-3 py-4 md:py-5 bg-white border !border-b-0 dark:bg-gray-700 rounded-tl-lg rounded-tr-lg">
        <div class="items-center justify-between sm:flex">
            <p class="text-sm font-bold text-gray-800 leading-wide">
                {{ __('messages.t_users') }}
            </p>
    ...
    <div class="bg-white dark:bg-zinc-800 overflow-y-auto border !border-t-0 !border-b-0 dark:border-zinc-600">
        <table class="w-full whitespace-nowrap old-tables">
            <thead class="bg-gray-200">
                <tr tabindex="0" class="w-full h-16 text-sm leading-none text-gray-800 focus:outline-none dark:text-white ">
            ...
            <tbody class="w-full">
                ...
                @foreach ($users as $user)
                    <tr class="text-sm leading-none text-gray-800 bg-white border-t border-b border-gray-100 focus:outline-none dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 dark:border-zinc-700/40" wire:key="users-{{ $user->id }}">

                        {{-- User --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('profile', $user->username) }}" target="_blank" class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="object-cover w-full h-full rounded lazy" src="{{ placeholder_img() }}" data-src="{{ src($user->avatar_id) }}" alt="{{ $user->username }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4">
                                    <p class="flex items-center text-xs font-medium">
                                        {{ $user->username }}
                                        @if ($user->status === 'verified')
                                            ...
                                            <div id="tooltip-account-verified-{{ $user->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                        ...
                        <td class="text-center">
                            @switch($user->status)
                                @case('active')
                                    <span class="px-4 py-1 text-xs font-semibold text-green-500 rounded-2xl bg-green-50">
                                        {{ __('messages.t_active') }}
                                    </span>
                                    @break
                                @case('pending')
                                    <span class="px-4 py-1 text-xs font-semibold rounded-2xl text-amber-500 bg-amber-50">
                                        {{ __('messages.t_pending') }}
                                    </span>
                                    @break
                                @case('banned')
                                    <span class="px-4 py-1 text-xs font-semibold text-gray-500 rounded-2xl bg-gray-50">
                                        {{ __('messages.t_banned') }}
                                    </span>
                                    @break
                                @case('verified')
                                    <span class="px-4 py-1 text-xs font-semibold text-blue-500 rounded-2xl bg-blue-50">
                        ...
                        {{-- Options --}}
                        <td class="text-center">
                            <div class="static inline-block text-left">
                                <div>
                                    <button data-dropdown-toggle="table-options-dropdown-{{ $user->id }}" type="button" class="inline-flex items-center justify-center w-8 h-8 bg-white rounded-full dark:bg-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-800 focus:outline-none focus:ring-0" id="menu-button" aria-expanded="true" aria-haspopup="true">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-600 dark:text-gray-400" viewBox="0 0 20 20" fill="currentColor"> <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z"/></svg>
                                    </button>
                                </div>
                                <div id="table-options-dropdown-{{ $user->id }}" class="absolute right-0 z-40 hidden mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg w-44 dark:bg-zinc-800 ring-1 ring-black ring-opacity-5 dark:divide-zinc-700 focus:outline-none" role="menu"  aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                    <div class="py-1" role="none">

                                        {{-- Edit --}}
                                        <a href="{{ admin_url('users/edit/' . $user->uid) }}" class="flex items-center px-4 py-2 text-sm text-gray-800 group" role="menuitem" tabindex="-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-400 ltr:mr-3 rtl:ml-3 group-hover:text-gray-500" viewBox="0 0 20 20" fill="currentColor"> <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/></svg>
                                            <span class="text-xs font-medium">{{ __('messages.t_edit_user') }}</span>
                                        </a>

                                        {{-- Balance --}}
                                        {{-- <a href="{{ admin_url('users/balance/' . $user->uid) }}" class="flex items-center px-4 py-2 text-sm text-gray-800 group" role="menuitem" tabindex="-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-400 ltr:mr-3 rtl:ml-3 group-hover:text-gray-500" viewBox="0 0 20 20" fill="currentColor"> <path d="M4 4a2 2 0 00-2 2v1h16V6a2 2 0 00-2-2H4z"/> <path fill-rule="evenodd" d="M18 9H2v5a2 2 0 002 2h12a2 2 0 002-2V9zM4 13a1 1 0 011-1h1a1 1 0 110 2H5a1 1 0 01-1-1zm5-1a1 1 0 100 2h1a1 1 0 100-2H9z" clip-rule="evenodd"/></svg>
                                            <span class="text-xs font-medium">{{ __('messages.t_user_balance') }}</span>
                                        </a> --}}

                                        {{-- Details --}}
                                        <a href="{{ admin_url('users/details/' . $user->uid) }}" class="flex items-center px-4 py-2 text-sm text-gray-800 group" role="menuitem" tabindex="-1">
                                        ...
                                        @if ($user->status === 'pending')
                                            <button wire:key="option-activate-{{ $user->id }}" wire:click="activate('{{ $user->id }}')" wire:loading.attr="disabled" wire:target="activate('{{ $user->id }}')" type="button" class="flex items-center px-4 py-2 text-sm text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group" role="menuitem" tabindex="-1" >
                                        ...
                                        @if ($user->status !== 'banned')
                                            <button wire:key="option-ban-{{ $user->id }}" x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_ban_this_user') }}') ? $wire.ban('{{ $user->id }}') : ''" wire:loading.attr="disabled" wire:target="ban('{{ $user->id }}')" type="button" class="flex items-center px-4 py-2 text-sm text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group" role="menuitem" tabindex="-1" >
                                        ...
                                        {{-- Delete user --}}
                                        <button wire:key="option-delete-{{ $user->id }}" x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_delete_this_user') }}') ? $wire.delete('{{ $user->id }}') : ''" wire:loading.attr="disabled" wire:target="delete('{{ $user->id }}')" type="button" class="flex items-center px-4 py-2 text-sm text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group" role="menuitem" tabindex="-1" >
