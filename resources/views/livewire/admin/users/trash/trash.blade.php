                    ...
                    @foreach ($users as $user)
                        <tr wire:key="trashed-users-{{ $user->uid }}">
                            ...
                            <td class="p-3 text-center">
                                <span
                                    class="text-gray-500 font-semibold text-[13px]">{{ format_date($user->deleted_at) }}</span>
                            </td>
                            ...
                            <td class="p-3 text-center">
                                <div class="space-x-4 rtl:space-x-reverse">
                                    ...
                                    {{-- Restore user --}}
                                    <button type="button" wire:click="confirmRestore('{{ $user->id }}')"
                                        data-tooltip-target="tooltip-action-restore-{{ $user->id }}"
                                    ...
                                    <div id="tooltip-action-restore-{{ $user->id }}" role="tooltip"
                                        class="inline-block absolute invisible z-10 py-2 px-3 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                    ...
                                    <button type="button" wire:click="confirmDelete('{{ $user->id }}')"
                                        data-tooltip-target="tooltip-action-delete-{{ $user->id }}"
                                        class="inline-flex justify-center items-center border font-semibold focus:outline-none w-8 h-8 leading-5 text-sm rounded border-gray-300 bg-white text-gray-800 shadow-sm hover:text-gray-800 hover:bg-gray-100 hover:border-gray-300 hover:shadow focus:ring focus:ring-gray-500 focus:ring-opacity-25 active:bg-white active:border-white active:shadow-none">
                                        <svg class="w-4 h-4" stroke="currentColor" fill="currentColor" stroke-width="0"
                                            viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path
                                    ...
                                    <div id="tooltip-action-delete-{{ $user->id }}" role="tooltip"
                                        class="inline-block absolute invisible z-10 py-2 px-3 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
