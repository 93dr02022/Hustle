        ...
        <table class="w-full whitespace-nowrap old-tables">
            ...
            <tbody class="w-full">
                ...
                @foreach ($orders as $order)
                    <tr class="text-sm leading-none text-gray-800 bg-white border-t border-b border-gray-100 focus:outline-none dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 dark:border-zinc-700/40" wire:key="orders-{{ $order->id }}">
                        ...
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('profile', $order->buyer?->username) }}" target="_blank" class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="object-cover w-full h-full rounded lazy" src="{{ placeholder_img() }}" data-src="{{ src($order->buyer?->avatar_id) }}" alt="{{ $order->buyer?->username }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4">
                                    <p class="flex items-center text-xs font-medium">
                                        ...
                                        @if ($order->buyer?->status === 'verified')
                                            ...
                                            <div id="tooltip-account-verified-{{ $order->id }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded-sm shadow-sm opacity-0 tooltip dark:bg-gray-700">
                        ...
                        <td class="text-center">
                            <div class="static inline-block text-left">
                                <div>
                                    <button data-dropdown-toggle="table-options-dropdown-{{ $order->id }}" type="button" class="inline-flex items-center justify-center w-8 h-8 bg-white rounded-full dark:bg-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-800 focus:outline-none focus:ring-0" id="menu-button" aria-expanded="true" aria-haspopup="true">
                                ...
                                <div id="table-options-dropdown-{{ $order->id }}" class="absolute right-0 z-40 hidden mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg w-44 dark:bg-zinc-800 ring-1 ring-black ring-opacity-5 dark:divide-zinc-700 focus:outline-none" role="menu"  aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                    <div class="py-1" role="none">

                                        {{-- Details --}}
                                        <a href="{{ admin_url('orders/details/' . $order->uid) }}" class="flex items-center px-4 py-2 text-sm text-gray-800 group" role="menuitem" tabindex="-1">
                                        ...
                                        {{-- Delete order --}}
                                        <button wire:key="option-delete-{{ $order->uid }}" x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_delete_this_order') }}') ? $wire.delete('{{ $order->id }}') : ''" wire:loading.attr="disabled" wire:target="delete('{{ $order->id }}')" type="button" class="flex items-center px-4 py-2 text-sm text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group" role="menuitem" tabindex="-1" >
