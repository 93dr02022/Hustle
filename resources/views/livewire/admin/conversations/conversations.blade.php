            ...
            <thead class="bg-gray-200">
                <tr tabindex="0"
                    ...
                    <th
                        ...
                        {{ __('messages.t_sender') }}</th>
                    <th
                        class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider ltr:text-left ltr:pl-4 rtl:text-right rtl:pr-4">
                        {{ __('messages.t_receiver') }}</th>
                    <th
            ...
            <tbody class="w-full">
                ...
                @foreach ($conversations as $c)
                    <tr class="focus:outline-none h-20 text-sm leading-none text-gray-800 bg-white dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 border-b border-t border-gray-100 dark:border-zinc-700/40"
                        wire:key="conversations-{{ $c->id }}">

                        {{-- From --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <div class="flex items-center">
                                <div class="w-10 h-10">
                                    <img class="w-full h-full rounded-sm object-cover lazy"
                                        src="{{ placeholder_img() }}" data-src="{{ src($c->from->avatar_id) }}"
                                        alt="{{ $c->from->username }}" />
                                </div>
                        ...
                        <td class="ltr:pl-4 rtl:pr-4">
                            <div class="flex items-center">
                                <div class="w-10 h-10">
                                    <img class="w-full h-full rounded-sm object-cover lazy"
                                        src="{{ placeholder_img() }}" data-src="{{ src($c->to->avatar_id) }}"
                                        alt="{{ $c->to->username }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4">
                            ...
                            <div class="relative inline-block text-left">
                                ...
                                <div id="table-options-dropdown-{{ $c->id }}"
                                    ...
                                    <div class="py-1" role="none">
                                        ...
                                        {{-- View messages --}}
                                        <a href="{{ admin_url('conversations/messages/' . $c->uid) }}"
                                            ...
                                            <span
                                                class="text-xs font-medium">{{ __('messages.t_read_messages') }}</span>
                                        </a>
