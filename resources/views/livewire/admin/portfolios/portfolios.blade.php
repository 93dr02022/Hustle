            ...
            <tbody class="w-full">
                ...
                @foreach ($portfolios as $p)
                    <tr class="focus:outline-none text-sm leading-none text-gray-800 bg-white dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 border-b border-t border-gray-100 dark:border-zinc-700/40"
                            ...
                            <a href="{{ url('projects', $p->slug) }}" target="_blank" class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="w-full h-full rounded object-cover lazy" src="{{ placeholder_img() }}"
                                        data-src="{{ src($p->thumb_id) }}" alt="{{ $p->title }}" />
                                </div>
                        ...
                        {{-- User --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('profile', $p->user->username) }}" target="_blank"
                                class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="w-full h-full rounded object-cover lazy" src="{{ placeholder_img() }}"
                                        data-src="{{ src($p->user->avatar_id) }}" alt="{{ $p->user->username }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4">
                        ...
                        <td class="text-center">
                            <div class="relative inline-block text-left">
                                <div>
                                    <button data-dropdown-toggle="table-options-dropdown-{{ $p->id }}"
                                        type="button"
                                ...
                                <div id="table-options-dropdown-{{ $p->id }}"
                                    ...
                                    <div class="py-1" role="none">
                                        ...
                                        {{-- Activate portfolio --}}
                                        @if ($p->status === 'pending')
                                            <button wire:key="option-activate-{{ $p->id }}"
                                                wire:click="activate('{{ $p->id }}')"
                                                wire:loading.attr="disabled"
                                                wire:target="activate('{{ $p->id }}')" type="button"
                                                ...
                                                <span
                                                    class="text-xs font-medium">{{ __('messages.t_activate_portfolio') }}</span>

                                            </button>
                                        @endif

                                        {{-- Delete portfolio --}}
                                        <button wire:key="option-delete-{{ $p->id }}"
                                            x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_delete_this') }}') ? $wire.delete('{{ $p->id }}') : ''"
                                            ...
                                            <span
                                                class="text-xs font-medium">{{ __('messages.t_delete_portfolio') }}</span>
