            ...
            <tbody class="w-full">
                ...
                @foreach ($refunds as $r)
                    <tr class="focus:outline-none text-sm leading-none text-gray-800 bg-white dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 border-b border-t border-gray-100 dark:border-zinc-700/40"
                        wire:key="refunds-{{ $r->id }}">

                        {{-- Item --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('service', $r->item->gig->slug) }}" target="_blank"
                                class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="w-full h-full rounded object-cover lazy" src="{{ placeholder_img() }}"
                                        data-src="{{ src($r->item->gig->image_thumb_id) }}"
                                        alt="{{ $r->item->gig->title }}" />
                                </div>
                        ...
                        {{-- Seller --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('profile', $r->seller->username) }}" target="_blank"
                                class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="w-full h-full rounded object-cover lazy" src="{{ placeholder_img() }}"
                                        data-src="{{ src($r->seller->avatar_id) }}"
                                        alt="{{ $r->seller->username }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4">
                        ...
                        {{-- Buyer --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('profile', $r->buyer->username) }}" target="_blank"
                                class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="w-full h-full rounded object-cover lazy" src="{{ placeholder_img() }}"
                                        data-src="{{ src($r->buyer->avatar_id) }}" alt="{{ $r->buyer->username }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4">
                        ...
                        <td class="text-center">
                            <span
                                class="text-[11px] font-normal text-gray-400 dark:text-gray-200">{{ format_date($r->created_at, 'ago') }}</span>
                        </td>
                        ...
                        <td class="text-center">
                            <div class="relative inline-block text-left">
                                <div>
                                    <a href="{{ admin_url('refunds/details/' . $r->uid) }}"
