<div class="w-full" x-data>
    <div class="grid grid-cols-12 md:gap-x-6 gap-y-6">
        ...
        <div class="col-span-12 lg:col-span-6">
            ...
            {{-- Reason --}}
            <div
                class="relative inline-block align-bottom bg-white rounded-lg px-4 pb-4 ltr:text-left rtl:text-right overflow-hidden shadow transform transition-all sm:align-middle sm:p-6 mb-8 w-full">
                <div>
                ...
                @if ($refund->status === 'rejected_by_seller' && $refund->request_admin_intervention)
                    <div class="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                        ...
                        {{-- Give refund --}}
                        <button
                            x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_approve_this_refund') }}') ? $wire.accept : ''"
                            ...
                            <div wire:loading wire:target="accept">
                                <svg role="status" class="inline w-4 h-4 text-white animate-spin" viewBox="0 0 100 101"
                        ...
                        {{-- Decline refund --}}
                        <button
                            x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_decline_this_refund') }}') ? $wire.decline : ''"
                ...
                <div class="border-t border-gray-200">
                    <dl>
                        ...
                        <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd
                                class="mt-1 text-[13px] font-medium text-primary-700 hover:text-primary-600 sm:mt-0 sm:col-span-2">
                                <a href="{{ url('service', $refund->item->gig->slug) }}"
                                    target="_blank">{{ $refund->item->gig->title }}</a>
                            </dd>
                        </div>
                        ...
                        <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd
                                class="mt-1 text-[13px] font-medium text-primary-700 hover:text-primary-600 sm:mt-0 sm:col-span-2">
                                <a href="{{ admin_url('users/details/' . $refund->buyer->uid) }}"
        ...
        <div class="col-span-12 lg:col-span-6">
            <div class="bg-white shadow-sm rounded-md overflow-hidden border border-gray-100">
                <div class="divide-y divide-gray-200">
                    <div class="px-4 py-5 sm:px-6">
                        <h2 class="text-base leading-6 font-medium text-gray-900">{{ __('messages.t_conversation') }}
                        </h2>
                    </div>
                    <div class="px-4 py-6 sm:px-6">
                        <ul role="list" class="space-y-8">
                            ...
                            @forelse ($messages as $msg)
                                <li>
                                    <div class="relative {{ !$loop->last ? 'pb-8' : '' }}">
                                        @if (!$loop->last)
                                            <span
                                                class="absolute top-5 ltr:left-5 rtl:right-5 ltr:-ml-px rtl:-mr-px h-full w-0.5 bg-gray-200"></span>
                                        @endif
                                                ...
                                                <div>
                                                    <div class="text-sm">
                                                        <a href="{{ url('profile', $msg->author_type === 'buyer' ? $msg->buyer->username : $msg->seller->username) }}"
                                                            target="_blank"
                                                            class="font-medium text-gray-900 hover:text-primary-600">{{ $msg->author_type === 'buyer' ? $msg->buyer->username : $msg->seller->username }}</a>
                                                    </div>
                            ...
                            @empty
                                <span
                                    class="text-center text-gray-400 block py-12 font-extrabold text-xs tracking-wider">{{ __('messages.t_no_messages_in_this_refund_conversation') }}</span>
                            @endforelse
