<div class="w-full">
    ...
    <div class="px-4 md:px-3 py-4 md:py-5 bg-white border !border-b-0 dark:bg-gray-700 rounded-tl-lg rounded-tr-lg">
        <div class="sm:flex items-center justify-between">
            ...
            <div>
                <a href="{{ admin_url('subcategories/create') }}"
            ...
            <thead class="bg-gray-200">
                <tr tabindex="0"
                    ...
                    <th
                        ...
                        {{ __('messages.t_subcategory') }}</th>
                    <th
                        class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider ltr:text-left ltr:pl-4 rtl:text-right rtl:pr-4">
                        {{ __('messages.t_parent_category') }}</th>
                    <th
            ...
            <tbody class="w-full">
                ...
                @foreach ($subcategories as $c)
                    <tr class="focus:outline-none text-sm leading-none text-gray-800 bg-white dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 border-b border-t border-gray-100 dark:border-zinc-700/40"
                        wire:key="subcategories-{{ $c->id }}">

                        {{-- Subcategory --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('categories/' . $c->parent->slug . '/' . $c->slug) }}" target="_blank"
                                class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="w-full h-full rounded object-cover lazy" src="{{ placeholder_img() }}"
                                        data-src="{{ src($c->icon_id) }}" alt="{{ $c->name }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4">
                                    <p class="font-medium text-sm">{{ $c->name }}</p>
                                    <p class="text-xs leading-3 text-gray-600 pt-2 max-w-xs truncate block">
                                        {{ $c->description }}</p>
                                </div>
                            </a>
                        </td>
                        ...
                        {{-- Parent category --}}
                        <td class="ltr:pl-4 rtl:pr-4">
                            <a href="{{ url('categories', $c->parent->slug) }}" target="_blank"
                                class="flex items-center">
                                <div class="w-8 h-8">
                                    <img class="w-full h-full rounded object-cover lazy" src="{{ placeholder_img() }}"
                                        data-src="{{ src($c->parent->icon_id) }}" alt="{{ $c->parent->name }}" />
                                </div>
                                <div class="ltr:pl-4 rtl:pr-4">
                                    <p class="font-medium text-sm">{{ $c->parent->name }}</p>
                                    <p class="text-xs leading-3 text-gray-600 pt-2 max-w-xs truncate block">
                                        {{ $c->parent->description }}</p>
                                </div>
                            </a>
                        </td>
                        ...
                        <td class="text-center">
                            <div class="relative inline-block text-left">
                                <div>
                                    <button data-dropdown-toggle="table-options-dropdown-{{ $c->id }}"
                                        type="button"
                                ...
                                <div id="table-options-dropdown-{{ $c->id }}"
                                    ...
                                    <div class="py-1" role="none">
                                        ...
                                        {{-- Edit subcategory --}}
                                        <a href="{{ admin_url('subcategories/edit/' . $c->uid) }}"
                                            ...
                                            <span
                                                class="text-xs font-medium">{{ __('messages.t_edit_subcategory') }}</span>
                                        </a>

                                        {{-- Delete subcategory --}}
                                        <button
                                            x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_delete_this') }}') ? $wire.delete('{{ $c->id }}') : ''"
                                            wire:loading.attr="disabled" wire:target="delete('{{ $c->id }}')"
                                            type="button"
                                            ...
                                            <span
                                                class="text-xs font-medium">{{ __('messages.t_delete_subcategory') }}</span>
