<header class="z-10 py-4 bg-white shadow-sm dark:bg-gray-800 h-16 fixed w-full">
    <div class="flex items-center justify-between h-full px-6 text-purple-600 dark:text-purple-300">
        ...
        <div class="h-full flex items-center">
            ...
            {{-- Mobile menu --}}
            <button
                class="p-1 ltr:mr-5 rtl:ml-5 -ml-1 rounded-md md:hidden focus:outline-none focus:shadow-outline-purple"
            ...
            <a href="{{ admin_url() }}" class="block h-full">
                <img src="{{ placeholder_img() }}" data-src="{{ src(settings('general')->logo_id) }}"
                    alt="{{ settings('general')->title }}" class="lazy h-full hidden md:block">
            </a>
        </div>
        ...
        <ul class="flex items-center flex-shrink-0 space-x-6 rtl:space-x-reverse">
            ...
            {{-- Notification button --}}
            <button
                class="h-8 w-8 flex items-center transition ease-in-out duration-150 justify-center rounded-md bg-slate-100 hover:bg-slate-200 relative"
                data-drawer-target="notifications-drawer" data-drawer-show="notifications-drawer"
                aria-controls="notifications-drawer" data-drawer-placement="right">
                ...
                <svg class="w-5 h-5 text-slate-500" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                        <path
                            ...
                            fill="currentColor" />
                        <rect fill="currentColor" opacity="0.3" x="10" y="16" width="4"
                            height="4" rx="2" />
                    </g>
                </svg>
            ...
            <li class="flex">
                ...
                <button
                    class="h-8 w-8 flex items-center transition ease-in-out duration-150 justify-center rounded-md bg-slate-100 hover:bg-slate-200 relative"
                    data-dropdown-toggle="admin-dropdown" data-dropdown-placement="bottom-start">
                    <svg class="w-5 h-5 text-slate-500" xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 24 24" version="1.1">
                        <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                ...
                <div id="admin-dropdown"
                    class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600 !mt-2">
                    <ul class="py-1 text-xs font-bold tracking-wide text-gray-700 dark:text-gray-200">

                        {{-- Fast support --}}
                        <li>
                            <a href="{{ url('https://wa.me/' . config('global.whatsapp') . '/?text=Hello') }}"
                                target="_blank"
