            ...
            <thead class="bg-gray-200">
                <tr tabindex="0"
                    ...
                    <th
                        ...
                        {{ __('messages.t_document_type') }}</th>
                    <th
                        class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center">
                        {{ __('messages.t_front_side') }}</th>
                    <th
                        class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center">
                        {{ __('messages.t_back_side') }}</th>
                    <th
                        class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center">
                    ...
                    <tr class="focus:outline-none text-sm leading-none text-gray-800 bg-white dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 border-b border-t border-gray-100 dark:border-zinc-700/40"
                        ...
                        <td class="text-center">
                            @if ($v->document_type === 'id')
                                <span
                                    class="text-xs font-bold tracking-wide text-gray-700">{{ __('messages.t_government_issued_id') }}</span>
                            @elseif ($v->document_type === 'driver_license')
                                <span
                                    class="text-xs font-bold tracking-wide text-gray-700">{{ __('messages.t_driver_license') }}</span>
                            @else
                                <span
                                    class="text-xs font-bold tracking-wide text-gray-700">{{ __('messages.t_passport') }}</span>
                            @endif
                        </td>

                        {{-- Frontside --}}
                        <td class="text-center">
                            <a href="{{ url('uploads/verifications/' . $v->uid . '/front/' . $v->file_front_side) }}"
                                target="_blank">
                                <img class="w-8 h-8 rounded object-cover mx-auto lazy" src="{{ placeholder_img() }}"
                                    data-src="{{ src($v->file_front_side) }}" />
                            </a>
                        </td>
                        ...
                        <td class="text-center">
                            @if ($v->backside)
                                <a href="{{ url('uploads/verifications/' . $v->uid . '/back/' . $v->file_back_side) }}"
                                    target="_blank">
                                    <img class="w-8 h-8 rounded object-cover mx-auto lazy"
                                        src="{{ placeholder_img() }}" data-src="{{ src($v->file_back_side) }}" />
                                </a>
                            @else
                                -
                            @endif
                        ...
                        <td class="text-center">
                            @if ($v->status === 'pending')
                                <div class="relative inline-block text-left">
                                    <div>
                                        <button data-dropdown-toggle="table-options-dropdown-{{ $v->id }}"
                                            type="button"
                                    ...
                                    <div id="table-options-dropdown-{{ $v->id }}"
                                        ...
                                        <div class="py-1" role="none">
                                            ...
                                            {{-- Approve --}}
                                            <button wire:key="option-approve-{{ $v->id }}"
                                                x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_approve_this_verification') }}') ? $wire.approve('{{ $v->id }}') : ''"
                                                wire:loading.attr="disabled"
                                                wire:target="approve('{{ $v->id }}')" type="button"
                                                ...
                                                <span
                                                    class="text-xs font-medium">{{ __('messages.t_approve_files') }}</span>

                                            </button>

                                            {{-- Decline --}}
                                            <button wire:key="option-decline-{{ $v->id }}"
                                                x-on:click="confirm('{{ __('messages.t_are_u_sure_u_want_to_decline_this_verification') }}') ? $wire.decline('{{ $v->id }}') : ''"
                                                wire:loading.attr="disabled"
                                                wire:target="decline('{{ $v->id }}')" type="button"
                                                class="text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group flex items-center px-4 py-2 text-sm"
                                                role="menuitem" tabindex="-1">
                                                ...
                                                <span
                                                    class="text-xs font-medium">{{ __('messages.t_decline_files') }}</span>
