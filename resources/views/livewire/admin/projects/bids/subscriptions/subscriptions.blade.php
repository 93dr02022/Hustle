            ...
            <div class="lg:flex lg:items-center lg:justify-between">
                ...
                <div class="mt-5 flex lg:mt-0 lg:ltr::ml-4 lg:rtl:mr-4">
                    ...
                    <span class="block lg:ltr:ml-3 lg:rtl:pr-3">
                        <a href="{{ admin_url('projects/bids') }}"
                ...
                <thead class="">
                    <tr class="bg-slate-200 dark:bg-zinc-600">
                        ...
                        <th
                            ...
                            @lang('messages.t_payment_method')</th>

                        {{-- amount --}}
                        <th
                            class="font-bold tracking-wider text-gray-600 px-5 py-4.5 text-center border-b-0 whitespace-nowrap text-xs uppercase dark:text-zinc-300 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md">
                            @lang('messages.t_amount')</th>
                ...
                <thead>
                    @forelse ($subscriptions as $s)
                        <tr class="intro-x shadow-sm bg-white dark:bg-zinc-800 rounded-md h-16"
                            wire:key="admin-dashboard-subscriptions-{{ $s->uid }}">
                            ...
                            <td
                                ...
                                @if ($s->payment_id)
                                    <span
                                        class="tracking-wider text-sm text-gray-700 truncate w-52">{{ $s->payment_id }}</span>
                                @else
                                    ...
                                    @if ($s->payment_method)
                                        @if (settings($s->payment_method))
                                            @if (settings($s->payment_method)->logo)
                                                <img class="h-3"
                                                    src="{{ src(settings($s->payment_method)->logo_id) }}"
                                                    alt="{{ $s->payment_method }}">
                                            @endif
                                            <span
                                                class="font-semibold text-sm text-gray-700">{{ settings($s->payment_method)->name }}</span>
                                        @else
                                            <span
                                                class="font-semibold text-sm text-gray-700">{{ $s->payment_method }}</span>
                                        @endif
                                    @else
                                        <span class="text-base text-gray-700">—</span>
                                    @endif
                            ...
                            <td
                                ...
                                @if ($s->status === 'paid')
                                    <span
                                        class="bg-green-100 text-green-600 px-4 inline-block leading-9 h-9 rounded-3xl font-medium text-xs whitespace-nowrap">
                                        ...
                                        <div
                                            ...
                                            <div class="flex items-center space-x-1 rtl:space-x-reverse">
                                                ...
                                                <div>
                                                    ...
                                                    {{-- Button --}}
                                                    <button type="button"
                                                        data-tooltip-target="tooltip-actions-approve-{{ str_replace('-', '_', $s->uid) }}"
                                                        id="modal-approve-payment-button-{{ str_replace('-', '_', $s->uid) }}"
                                                    ...
                                                    {{-- Tooltip --}}
                                                    <x-forms.tooltip
                                                        id="tooltip-actions-approve-{{ str_replace('-', '_', $s->uid) }}"
                                                ...
                                                <div>
                                                    ...
                                                    {{-- Button --}}
                                                    <button type="button"
                                                        data-tooltip-target="tooltip-actions-reject-{{ str_replace('-', '_', $s->uid) }}"
                                                        id="modal-reject-payment-button-{{ str_replace('-', '_', $s->uid) }}"
                                                        class="flex items-center justify-center h-6 w-6 rounded-full border border-transparent bg-white text-yellow-600 hover:bg-transparent hover:border-yellow-600">
                                                        <svg class="w-4 h-4" stroke="currentColor" fill="currentColor"
                                                            stroke-width="0" viewBox="0 0 24 24"
                                                            xmlns="http://www.w3.org/2000/svg">
                                                    ...
                                                    {{-- Tooltip --}}
                                                    <x-forms.tooltip
                                                        id="tooltip-actions-reject-{{ str_replace('-', '_', $s->uid) }}"
                            ...
                            <td
                                ...
                                <div class="flex items-center justify-center space-x-4 rtl:space-x-reverse">
                                    ...
                                    <div>
                                        <button type="button"
                                            data-tooltip-target="tooltip-actions-preview-{{ $s->bid->uid }}"
                                            id="modal-preview-bid-button-{{ $s->bid->uid }}"
                                        ...
                                        <x-forms.tooltip id="tooltip-actions-preview-{{ $s->bid->uid }}"
                                    ...
                                    <div>
                                        <button type="button"
                                            data-tooltip-target="tooltip-actions-delete-{{ str_replace('-', '_', $s->uid) }}"
                                            id="modal-delete-payment-button-{{ str_replace('-', '_', $s->uid) }}"
                                            class="inline-flex justify-center items-center border font-semibold focus:outline-none w-8 h-8 leading-5 text-sm rounded border-gray-300 bg-white text-gray-800 shadow-sm hover:text-gray-800 hover:bg-gray-100 hover:border-gray-300 hover:shadow focus:ring focus:ring-gray-500 focus:ring-opacity-25 active:bg-white active:border-white active:shadow-none">
                                            <svg class="w-4 h-4" stroke="currentColor" fill="currentColor"
                                                stroke-width="0" viewBox="0 0 24 24"
                                                xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <x-forms.tooltip
                                            id="tooltip-actions-delete-{{ str_replace('-', '_', $s->uid) }}"
                                            :text="__('messages.t_delete_payment')" />
                                    </div>
                        ...
                        {{-- Preview modal --}}
                        <x-forms.modal id="modal-preview-bid-container-{{ $s->bid->uid }}"
                            target="modal-preview-bid-button-{{ $s->bid->uid }}"
                            uid="modal_preview_bid_{{ $s->bid->uid }}" placement="center-center" size="max-w-2xl">
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="mb-8">
                                            ...
                                            <div class="flex items-center space-x-4 rtl:space-x-reverse">
                                                ...
                                                {{-- Avatar --}}
                                                <a href="{{ url('profile', $s->bid->user->username) }}"
                                                    class="block">
                                                    <img class="rounded-full h-12 w-12 object-cover object-center lazy"
                                                        src="{{ placeholder_img() }}"
                                                        data-src="{{ src($s->bid->user->avatar_id) }}"
                                                        alt="{{ $s->bid->user->username }}">
                                                </a>
                                                    ...
                                                    <div
                                                        ...
                                                        @if ($s->bid->user->country)
                                                            <p class="flex items-center space-x-1 rtl:space-x-reverse">
                                                                <img class="h-4 ltr:pr-0.5 rtl:pl-0.5 -mt-0.5 lazy"
                                                                    src="{{ placeholder_img() }}"
                                                                    data-src="{{ countryFlag($s->bid->user->country->code) }}"
                                                                    alt="{{ $s->bid->user->country->name }}">
                                                        ...
                                                        <p
                                                            ...
                                                            <svg aria-hidden="true"
                                                                class="w-4 h-4 {{ $s->bid->user->rating() == 0 ? 'text-gray-400' : 'text-amber-500' }} -mt-0.5"
                        ...
                        {{-- Approve modal --}}
                        <x-forms.modal id="modal-approve-payment-container-{{ str_replace('-', '_', $s->uid) }}"
                            target="modal-approve-payment-button-{{ str_replace('-', '_', $s->uid) }}"
                            uid="modal_approve_payment_{{ str_replace('-', '_', $s->uid) }}"
                            ...
                            <x-slot name="footer">
                                <div class="flex justify-between items-center w-full">
                                    ...
                                    {{-- Accept --}}
                                    <button type="button" wire:click="approve('{{ $s->id }}')"
                        ...
                        {{-- Reject modal --}}
                        <x-forms.modal id="modal-reject-payment-container-{{ str_replace('-', '_', $s->uid) }}"
                            target="modal-reject-payment-button-{{ str_replace('-', '_', $s->uid) }}"
                            uid="modal_reject_payment_{{ str_replace('-', '_', $s->uid) }}" placement="center-center"
                            size="max-w-md">
                            ...
                            <x-slot name="footer">
                                <div class="flex justify-between items-center w-full">
                                    ...
                                    {{-- Reject --}}
                                    <button type="button" wire:click="reject('{{ $s->id }}')"
                                        wire:loading.attr="disabled"
                        ...
                        {{-- Delete modal --}}
                        <x-forms.modal id="modal-delete-payment-container-{{ str_replace('-', '_', $s->uid) }}"
                            target="modal-delete-payment-button-{{ str_replace('-', '_', $s->uid) }}"
                            uid="modal_delete_payment_{{ str_replace('-', '_', $s->uid) }}" placement="center-center"
                            size="max-w-md">
